// 單行 CSV 解析與跳脫(RFC 4180 雙引號規則)— 用於頻次 / NextWord 關聯資料的匯出匯入。
// parseLine + escape 為對等對應,確保 round-trip 不失真。
...
object DictionaryCsvCodec {
    ...
    // 逐 (word, tl, count) 列編成 word,tl,count\n;word 與 tl 走 escape() 轉義。
    ...
    // 3 欄 (word,tl,count) 現格式;2 欄 (word,count) 舊檔 → tl="";其餘欄數跳過。
