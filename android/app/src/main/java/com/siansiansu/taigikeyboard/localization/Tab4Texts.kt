...
object Tab4Texts {
    ...
    val resetFailed = LocalizedText(hanji = "恢復設定失敗，請重試")
    val noEmailApp = LocalizedText(hanji = "揣無 Email App")
    ...
    val tpsOrMapsToERInfo =
        LocalizedText(
            hanji = "台羅 or 毋是正式寫法，方音符號 ㄜ 正式干焦對應 er。共 or 嘛對應 ㄜ 是就音值來處理，毋過會予 er 佮 or 兩个音位攏對應到仝一个 ㄜ。\n\n開啟（預設）：or 對應 ㄜ，候選詞排佇 er 後壁。\n關閉：or 對應 ㄛ（恢復台羅 o），避免 er／or 相濫。教典 or 攏有 o 版本，袂影響拍字。",
        )

    // MARK: - 裝置資訊

    val diagnosticSectionTitle = LocalizedText(hanji = "裝置資訊")
    val diagnosticCopy = LocalizedText(hanji = "Khó͘-phih 裝置資訊")
    val diagnosticCopied = LocalizedText(hanji = "已 khó͘-phih")
    val diagnosticShare = LocalizedText(hanji = "分享裝置資訊")
    val diagnosticEmail = LocalizedText(hanji = "Email 回報問題")
