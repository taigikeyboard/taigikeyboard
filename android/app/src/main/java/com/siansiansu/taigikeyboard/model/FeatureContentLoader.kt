...
object FeatureContentLoader {
    ...
    private var cachedFeatures: List<FeatureContent>? = null
    private var cachedFAQs: List<FeatureContent>? = null

    fun loadFeatures(context: Context): List<FeatureContent> {
        cachedFeatures?.let { return it }
        val json = context.assets.open("tab1-features.json").bufferedReader().use { it.readText() }
    ...
    fun loadFAQs(context: Context): List<FeatureContent> {
        ...
        val json = context.assets.open("tab1-faq.json").bufferedReader().use { it.readText() }
        val faqs = parseContentArray(JSONObject(json), "faqs")
        cachedFAQs = faqs
        return faqs
    }

    private fun parseContentArray(root: JSONObject, key: String): List<FeatureContent> {
    ...
    private fun parseAttachment(obj: JSONObject): ParagraphAttachment {
        return when (val type = obj.getString("type")) {
            ...
            "image" -> ParagraphAttachment.Image(name = obj.getString("name"))
            "link" -> ParagraphAttachment.Link(
                text = parseLocalizedText(obj.getJSONObject("text")),
