// Phonetics + Derivation + TPS bridge — 15 ops + toneVariations cache.
// Mirrors iOS RustEngineBridge+Phonetics.swift (TPS merged into same file per simplify decision).
// Uses RustEngineBridge.sendRawBytes(...) for the JNI roundtrip to preserve the pre-split facade
// dispatch path's parse-fail behavior (JNI exceptions propagate, parseFrom failure returns null).
// Do not switch to dispatchRaw wrapped in try/Throwable — it changes op-name recordFailure and exception semantics.
