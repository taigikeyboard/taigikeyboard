...
class TaihoaProcessor(BaseProcessor):
    def __init__(self):
        """初始化台華雙語辭典處理器，設定統計計數器與跳過項目清單"""
        super().__init__()
...
def main():
    """主程式：讀取台華雙語辭典 CSV 檔案，處理資料並輸出為 CSV 格式"""
    # 設定 logging 寫入檔案
    logging.basicConfig(
        level=logging.INFO,
        format='%(message)s',
        handlers=[
            logging.FileHandler('log/taihoa.log', mode='w', encoding='utf-8')
        ]
    )
    logger = logging.getLogger(__name__)

    processor = TaihoaProcessor()
    input_file = Path('raw/ChhoeTaigi_TaihoaSoanntengTuichiautian.csv')
    output_file = Path('csv/taihoa.csv')
