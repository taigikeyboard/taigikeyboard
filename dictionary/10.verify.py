...
class DictionaryVerifier:
    ...
    def check_empty_or_missing(self, row, row_num):
        """檢查空白或缺失的欄位"""
        issues = []
        required_fields = ['tl', 'poj', 'tl_no_tone', 'poj_no_tone']
    ...
    def check_consistency(self, row, row_num):
        ...
        try:
            ...
            actual_tl_syllables = len([s for s in re.split(r'[-\s]+', tl) if s])

            if syllable_count != actual_tl_syllables:
                issues.append({
                    'type': '音節數不符',
                    'field': 'syllable_count',
                    'expected': actual_tl_syllables,
                    'actual': syllable_count,
                    'tl': tl,
                    'row': row_num
                })
        except ValueError:
...
def main():
    ...
    logging.basicConfig(
        ...
        format='%(message)s',
        handlers=[
            logging.FileHandler('log/verify.log', mode='w', encoding='utf-8')
        ]
    )
    logger = logging.getLogger(__name__)
