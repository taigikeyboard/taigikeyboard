...
DEFAULT_CSV = BASE_DIR / "output" / "dictionary.csv"
...
class DictionaryVerifier:
    ...
    def print_issue_detail(self, issue, logger):
        ...
        if issue['type'] == '非法字符':
            ...
            logger.info(f"  {row_info} 漢字音節數不符: 漢字 '{issue['hanzi']}' ({issue['hanzi_count']} 字) "
...
def main():
    ...
    parser.add_argument('file', nargs='?', default=str(DEFAULT_CSV),
                       help=f'要驗證的 CSV 檔案（預設：{DEFAULT_CSV.relative_to(BASE_DIR.parent)}）')
