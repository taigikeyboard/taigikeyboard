...
- If a fix touches Rust under `engine/`, run `make build` (iOS xcframework + Android jniLibs + macOS xcframework + all platform protos) and include every regenerated artifact in the commit. The project has no GitHub Actions CI for Rust — the local Makefile gate is canonical — so never push without rebuilding. The rebuild itself is Tier A even when the source change is Tier C — bundle if same batch, otherwise separate commit.
...
- Per `~/.claude/rules/round-workflow.md` § Branching & rounds: each Codex sweep is its own coding round when fixes are applied — bundle the swept fixes into commits within the same PR (do NOT open a new PR per finding).
