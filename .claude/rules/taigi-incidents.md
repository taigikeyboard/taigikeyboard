...
- **Release scope is USER-gated**: never write "documented known limitation", "post-vX", "deferred" or "ready to tag" without the USER's dated words (USER: "do not decide on your own what is outside v3.5.8 scope; I will give explicit instructions when v3.5.8 should be released").
...
- **Account-bound tooling stays private**: skills or scripts tied to a personal account (mailbox triage, chat-server triage, OAuth clients, runtime state) live in the maintainer's private dotfiles repo, never under `.claude/`. A doc never spells out a personal address or an account / project / server ID — "a former work address", "the maintainer's author email". `personal-email` (`.gitleaks.toml`) and the private denylist (`scripts/private-denylist-scan.sh`) gate commits; neither sees PR bodies or issues, so check those by hand. History is not rewritten (USER 2026-09-24: "do not rewrite history, but I want to avoid this kind of thing in the future").
