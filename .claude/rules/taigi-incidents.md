...
- **S23 TPS 無調連續輸入的鼻音替代讀法** (2026-08-19, §35 parts B/C): 無聲調、無空白直接打 —— 考卷 `ㄎㄛ`+`ㄍ`+`ㄫ` → 考卷 (khó-kǹg); 毋是 `ㄇ`+`ㄒ`+`ㄧ` → 毋是 (不必長按 ㆬ)。組字區仍顯示 auto-correct 的字面 (`ㄎㄛㆻㄫ` / `ㄇㄒㄧ`) —— 這是 span-local 替代讀法,不改 raw buffer。**負對照必須同時驗**: 門 `ㄇ`+`ㄫ` → 門 (mn̂g)、兩 `ㄋ`+`ㄫ` → 兩 (nn̄g)、博雅 `ㄆ`+`ㆦ`+`ㄍ`+`ㄫ`+`ㄚ` → 博雅 (phok-ngá) 三者不可消失; 交代 / 龜毛 / 雞胸 / 烏白 / 包袱 (S18-S21) 不變; `ㄎㄛㆻ`␣`ㄫ` (中間有空白) **不可**出現考卷; 單打 `ㄇ` 或 `ㄫ` 仍列出 m- / ng- 開頭詞 (partial-prefix 分支未被關掉)。iOS+Android engine-only,需 `make build`,免 `make dict`。macOS 只出 TL/POJ 無 TPS 介面故不受影響。⚠ 同一封回報信 (zw 2026-06-22 v3.6.3) 的另一半 —— 方音齒盤第一/四調無法用空白鍵揀調 —— **不在本輪**: 空白只是分隔符無聲調語意,且 `tps:` FST namespace 無法表達「只有第一調」,另開一輪。
