...
Both docs must comply with `rules/rust-best-practices.md` (workspace layout §1, error handling §2, crate choices §3, non-goals §8) and `rules/rust-ffi-safety.md` (FFI boundary discipline §1, `unsafe` discipline §3, opaque handle pattern §4). Rule deviations require `// JUSTIFICATION:` prose in the spec.
