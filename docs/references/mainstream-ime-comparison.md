...
> **Purpose**: Centralised cross-reference of every mainstream IME / keyboard repo cloned under `references/`, plus a few external projects worth knowing. Read this **before** writing a `Best practices alignment` section in a plan, before designing a new engine slice, or before asserting "Project X already does Y".
...
| 10 | `rime-moetaigi/` | YAML + dict | Cross (RIME schema) | Schema data | DAG inherits from librime | Frequency from MOE corpus + 楊允言 word stats | Auto user-phrase via librime | TPS (Bopomofo-style) + 9 tone marks | CC0 | **Only mainstream RIME-based Taigi schema**. Reference for keyboard layout, tone-mark placement, Mandarin reverse-lookup mode, TPS-display mode |
...
| 21 | `PhahTaigi_iOS/` | Swift (Realm + RxSwift) | iOS (KeyboardExt) | Full IME (Taigi) | Prefix lookup over Realm dict; no lattice | Realm dict frequency | None on-device | POJ/TL (tone-optional) | GPL-3.0 | **Closest iOS Taigi-IME prior art** — keyboard-extension structure, Realm dict store, Han-lo mixed candidates. The "Hanji-with-romanization commits romanization in one tap" UX our §34/S22 literal-roman candidate mirrors ("PhahTaigi parity"). Stale (`bd91e46`, 2022-03-03) |
...
4. **`rime-phah-taibun`** — second RIME Taigi schema (#22): POJ/TL-unified spelling (no mode switch), tone-optional input, Han/Roman mixed output, Mandarin reverse-lookup; ~220K-entry corpus-merge pipeline.
5. **`PhahTaigi_iOS`** — iOS Taigi-keyboard prior art (#21): keyboard-extension structure, Realm dict store, one-tap Hanji-with-romanization commit ("PhahTaigi parity" for §34/S22).
...
- **Inspiration takeaways**:
  ...
  - Mandarin reverse-lookup mode UX (we have this as a roadmap consideration)
...
- **Inspiration takeaways**:
  ...
  - **Han-lo mixed candidates** (`VT_MIXED`) — show Hanji + TL in the same candidate. We do this.
...
- **Where to look**:
  ...
  - `ModulePackages/` — `OVAFHomophoneLookup`, `OVAFReverseLookup`, `OVOFHanConvert` (Simplified/Traditional), `YKAFOneKey`, `YKAFPhraseAware`, `OVAFBopomofoCorrection` — the "around-filter" / "output-filter" plugin taxonomy
  - `Loaders/OSX-IMK/CV*.mm` — 2012 Cocoa candidate / symbol / smiley / notify windows (superseded by #26 and our `macos/`)
- **Inspiration takeaways**: (1) `BISHandleKey`'s one-round-trip result shape (commit + composition + reading + candidate page + cursor + verb) as a checklist for our TSF `Runtime` response; (2) `actionVerb` / `actionParameter` as an escape hatch for "engine asks host to do X" (open preferences, send fake key) instead of growing the RPC surface; (3) around-filter / output-filter plugin split (`OVAF*` / `OVOF*`) as a naming model for post-processing stages (Simplified/Traditional, full-width, anti-Zhuyinwen); (4) Takao `DatabaseCooker` as the reference shape for a corpus → LM → installer Makefile graph.
