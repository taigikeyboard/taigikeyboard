...
- What **is** worth learning sits around the engine, and most of it targets exactly the desktop concerns we own on macOS / Windows:
  ...
  9. **User-facing input features** on the product page (selection learning, add word `Shift+←`/`Ctrl+1…9`, symbol table, Simplified output toggle, `Tab` word break, input modules) traced to source and compared with ours (§9). Two gestures we lack: in-buffer word capture and forced phrase break; both desktop-only, unscheduled.
...
**Takeaway**: the same label-noise problem exists for Taigi (hanji → TL is ambiguous: `重/tîng` vs `重/tāng`, the (Hanji, TL) identity in Core Principle #6). If a numeric ranking harness is ever requested, ChiaKey's strict / loose split and "selections per pass" replay are the design. Not a proposal; reference only.
