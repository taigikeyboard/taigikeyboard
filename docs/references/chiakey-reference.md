...
- **Associated phrases** (`ModulePackages/OVIMMandarin/OVAFAssociatedPhrase.cpp`): head-character keyed `associated_phrases` SQLite table, offered after commit. Simpler than our next-word booster; only the "toggle rebuilds contexts" contract is notable (§2).
