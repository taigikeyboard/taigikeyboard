...
- ~~D4 candidate-selection keys vs numeric tone digits~~ — **CLOSED 2026-08-15**: USER took
  ...
  slot keys are now the bare keys `q w d f z x v y ;` (slots 1–9) with `⇧1…⇧9` also picking
  under every set — `⌃1…9` / `⌥1…9` remain as picker choices (`CandidateSlotKeySet`). A bare digit
