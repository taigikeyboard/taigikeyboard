...
**Scope**: Rust `engine/phonetics::tps` + `tps_adjust` + `api` (since v3.5.1 PR #186). Bridged via `RustEngineBridge.tlNumericToTPS` / `tlDisplayToTPS` / `tpsInputAdjust` / `containsTPS` / `isTPSToneMark`. The TPS → TL direction (`phonetics::tps_to_tl`) is Rust-internal only — consumed by `lexicon::classify_input`, no FFI surface after the dead-surface cleanup (2026-05-04). iOS / Android `TPS*.swift` / `.kt` files were deleted under Path G.
...
**Status**: Pinned during Item 4 P3 Phase D — the round that retires the View-subclass keyboard body for a Compose tree consuming P2's `KeyboardLayoutData` + `KeyboardLayoutSolver`. The behaviors below are platform-Android by nature (touch dispatch, `PopupWindow` ownership, `WindowInsets` consumption) — there is no iOS mirror because the iOS keyboard is a KeyboardKit-driven SwiftUI tree with its own platform contract. The `INVARIANT_keyboard_*` labels nonetheless live here so that any future cross-platform "keyboard body" invariant lookup lands on a single canonical doc.

**Why**: Phase D structurally retires the dismiss-bug hazard documented in `auto-memory/project_ime_window_arch.md`. Any drift on these invariants risks resurrecting that class of bugs (off-by-pixel ghost taps, pinned-popup latency drift, lost multi-touch cancellation, navbar-area inset re-tunes that re-introduce `MATCH_PARENT × MATCH_PARENT` + `TOUCHABLE_INSETS_VISIBLE` pairing).
...
`TextInputManager.onRegisterInputView` resolves `textViewGroup`, mounts the keyboard ComposeView, and registers all four smartbar overlays **synchronously** before returning. Only the layout-data fetch (`ensureLayoutLoaded`) runs on a background coroutine. This ordering is load-bearing because `InputMethodService.onWindowShown` calls `setActiveInput(R.id.text_input)` immediately after the IME window first becomes visible, and that call reads `textInputManager.textViewGroup` synchronously. If the resolution sat behind a `launch(Dispatchers.Default) { withContext(Main) { ... } }` thread-hop, `onWindowShown` could race ahead, find `textViewGroup == null`, and `ViewGroup.indexOfChild(null) == -1` would feed `ViewAnimator.setDisplayedChild(-1)`, which wraps to `childCount - 1` (= the `media_input` emoji keyboard) — the "first-install opens emoji keyboard" symptom. `TaigiKeyboard.setActiveInput` defends against the same class of bug by clamping the resolved index with `coerceAtLeast(0)`.
