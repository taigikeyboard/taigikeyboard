...
**And it draws a faint fill of its own**: the literal cell is what the user is currently typing, not a candidate the engine offered, so it is tinted to read as a different KIND of row (USER 2026-09-09) — macOS `NSColor.quaternarySystemFill` (a system fill, dynamic per appearance, resolved inside the panel's own `effectiveAppearance`), Windows WinUI's `SubtleFillColorSecondary` (`Theme.literal_fill`, `None` under a high-contrast scheme, which is chosen to be free of decorative cards). Precedence is selection > literal tint > nothing, and only the SELECTION changes the text colours — so a highlighted literal cell (the fresh bar, which selects cell 0) is drawn like any other highlighted cell and the tint cannot read as a second selection.
