...
- **The slot key beside a cell is the key that picks it.** macOS draws, beside each visible slot, the most direct key that picks it right now (`CandidateIndexLabel`, PR #604 — superseding the 2026-08-21 "no index column" ruling once bare digits could select): a bare `1`…`9` where the digit cannot be a tone marker or the user has latched selection with `↓`; otherwise the key the chosen set gives the slot — `q w d f z x` then `⇧7 ⇧8 ⇧9` under the shipped letter set, or `⌃n` / `⌥n` under the digit sets (`CandidateSlotKeySet`, USER 2026-08-28). `⇧1`…`⇧9` pick under every set and are drawn only where they are the most direct key. The label column is measured against every form so it never steps as the live key changes. The slot numbering still follows the viewport. **Upgrade edge (accepted 2026-08-28)**: the fixed `⇧` digits are read off the number-row key code, so the recorder refuses them and a global row left holding one from before is cleared at launch; a *composing* row recorded as the symbol `⇧3` types (`#` with Shift, US layout) cannot be told apart from a genuine `⇧#` binding without a key code, is left in place, and is shadowed by the fixed tier.
