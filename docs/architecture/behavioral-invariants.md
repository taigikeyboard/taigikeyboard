...
- **The two scripts are the only thing a cell draws.** macOS binds `⌃1`…`⌃9` to the visible slots (bare digits are unavailable — they are the numeric tone markers of TL and POJ) but does NOT draw the chord beside each candidate: a modifier badge on every row is noise the reader looks past, and it competes with the second script for the width that makes both readable (USER 2026-08-21). The chords keep working, and the slot numbering still follows the viewport.
