...
**A second USER-requested exception (2026-08-30 「不需要分成 title/subtitle，漢字和羅馬字會一併出現」, implemented 2026-09-01): 候選詞顯示 = 漢羅合用.** The cell keeps BOTH scripts but breaks the "primary / secondary, not a formatted string" clause on purpose: one label, `漢字 羅馬字` (hanji leads, single ASCII space, one font), no subtitle / annotation. Committing writes the hanji (the cell IS the annotation; the document stays clean), so 括號標註 still applies as `漢字 (羅馬字)` and the 文/A key is inert. The platforms express this by projecting it onto the existing pair — effective `isTranslateSwapped = true`, `isOutputBothScripts = stored` — a **compatibility projection** of "cell leads with hanji, commit writes hanji", not a claim that 合用 is hanji-first in any deeper sense; every reader of the pair (auto-space, full-width, 選/suán, the nextword Enter-raw gate `decide.rs:93`) therefore behaves exactly as today's hanji-first mode, which also shows the roman and also skips that recording. No engine reader: a combined cell is still distinct by (hanji, roman), so neither §44 pass runs. Cell height stays two-line on iOS/Android (no one-line repack); only the width measurement changes to the combined label. Research: the same report, Part I §1-§11 (Q1-Q8 resolved by their recommended options, auto mode).
