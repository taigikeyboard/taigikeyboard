...
**Toggle (顯示羅馬字 setting, §34/S22) — mobile only**: on iOS / Android the candidate is user-toggleable, **default OFF since #445 (USER 2026-06-19)** — shipped default ON in #398, flipped to opt-in. When the user turns the **顯示羅馬字** setting OFF, the index-0 forced prepend is skipped. The toggle gates **only** this §34 WYSIWYG prepend — roman-only / OOV-synth candidates that `assemble_candidates` produces on their own are unaffected (turning it off does NOT remove all romanization candidates). Wire path: the platform setting is **positive** (`isLiteralRomanCandidateEnabled` iOS / `literalRomanCandidateEnabled` Android, default `false` since #445); `ComposingManager` inverts it into `FetchAtPos.literal_roman_candidate_disabled` (a `bool`, field 6) once per fetch and passes it to both fetch phases. The proto field is **inverted** so proto3-default `false` = show = legacy always-on (mirrors the `enabled_sources_bitmask` legacy-default sentinel) — un-wired builds and proto-decoded fixtures keep the candidate. `handle_fetch_at_pos` gates the prepend on `!literal_roman_candidate_disabled`. The toggle is surfaced both in the host-app Settings tab (拍字設定 section) and the in-keyboard quick-settings overlay on both mobile platforms. CROSS-PLATFORM INVARIANT — `EngineSettings.isLiteralRomanCandidateEnabled` (iOS `Settings/EngineSettings.swift` ↔ Android `ime/core/settings/EngineSettings.kt`).

**Desktop policy (macOS / Windows) — always on, no setting** (USER 2026-09-02「第一個位置永遠顯示正在輸入的字」): the shared engine gate is unchanged; the desktop bridges pin `FetchAtPos.literal_roman_candidate_disabled = false` on every fetch (macOS `Engine/RustEngineBridge+Composing.swift` `composingFetchAtPos`, Windows `taigi-windows-core/src/engine/composing.rs` `fetch_at_pos`) and carry no `isLiteralRomanCandidateEnabled` / `literalRomanCandidateEnabled` setting at all. The macOS pane row had already gone in #558 and `RetiredSettingsCleanup` wiped the key each launch, so the desktop candidate had been permanently OFF; the key name now lives in `retiredDefaultsNames` as hygiene (macOS only — Windows keeps no retired-key sweep, so a stale `literalRomanCandidateEnabled` in its `settings.json` is simply unread, as every retired Windows key is). The `literalRomanCandidate` i18n string is scoped to the mobile platforms. Because the candidate window opens with index 0 highlighted and Return = `confirmHighlighted`, **Return commits the typed literal** and the first dictionary candidate moves to the second slot key (`w` on the default set) / `→`; ⇧Return (`commitLiteral`, raw `CommitRaw`) stays as the raw escape hatch that ignores the highlight. Space on the literal cell (one script) still answers `.ignored` (§42). Under 羅馬字 (§44) the literal collapses a same-roman dictionary row into itself and keeps slot 0; under 漢羅濫 (S27) the literal cell leads and the first Hanji cell follows. Named platform divergence per `cross-platform-alignment.md` §3: mobile has the strip-space trade-off the toggle exists for; the desktop window does not. ⚠ Pre-existing gap surfaced by this policy (not fixed here, Codex pre-impl 2026-09-02): `AutoSpacePolicy` (macOS `macos/Sources/TaigiInputMethodCore/Controller/AutoSpacePolicy.swift`, Windows `windows/crates/taigi-windows-core/src/policies/auto_space.rs`) decides the space from `CandidateScript` + the effective swap, and a one-script literal is presented as `.primary`, so Return on the literal under 漢字-first side-by-side or 漢羅濫 may NOT earn the auto-space a romanization commit should — the roman-first and 羅馬字 modes do.
...
**Scope**: shared Rust engine (`composing`) for the candidate itself + the toggle gate; thin platform wiring for the setting on the two mobile platforms (settings store + `EngineSettings` + `ComposingManager` invert + UI toggle); the desktop bridges pin the field. The injection lives in the display-layer seam `composing::dispatch::handle_fetch_at_pos` (`literal_roman_candidate` helper), NOT in the segmentation / cost primitive `assemble_candidates` (incidents S5 / §18 / S9 — display-layer change). Both platforms already commit a roman-only (hanji-absent) candidate's `roman` in swapped mode (iOS `formatOutputText` `else` / Android `CandidateClickHandler` `when` `else`). Needs `make build` to refresh xcframework/jniLibs before device dogfood (no `make dict` — dict artifacts unchanged).
