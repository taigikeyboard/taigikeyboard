...
## 35. TPS alternate-reading enumerate — a locally-ambiguous glyph reading surfaces its hidden word
...
- **Mechanism**: `build_continuous_keys` canonicalizes the raw buffer ONCE, builds the base reading's lattice, then one lattice per ALTERNATE reading (`alternate_canonicals`) and appends their left-anchored keys after the base reading's; all of them are fetched in the single `fetch_candidates_for_keys` batch. Every substitution is a 3-byte Bopomofo → 3-byte Bopomofo glyph swap, so it is **byte-length preserving** — `canonical_to_raw_end` and the downstream offset maps stay valid and each alternate's keys map to the same raw span.
- **Three generators** (each yields at most one alternate, so the set is bounded at three by construction — no explosion guard needed):
  - **A — coda→onset de-fold** (`defold_single_coda`): a folded coda glyph that is really the NEXT syllable's onset. 雞胸 `ㄍㆤㆷㄧㄥ` → `ㄍㆤㄏㄧㄥ`. Eligible = `phonetics::defold_coda_to_initial` is `Some` AND the next char is `phonetics::is_tps_vowel_material`; `None` for zero or >1 site.
  - **B — stranded-nasal terminalization** (`terminalize_stranded_nasals`): an onset-form nasal `ㄇ`/`ㄋ`/`ㄫ` that cannot open a syllable at its position reads as its terminal glyph (`phonetics::nasal_final_form`, the narrow nasal window onto the same private `dual_final_form` the keystroke path uses). 毋是 `ㄇㄒㄧ` → `ㆬㄒㄧ`. Needed because the per-keystroke `syllabic_nasal_replacement` fires ONLY when the next keystroke is a tone mark — toneless continuous input never supplies that trigger, and for a buffer-final nasal no later keystroke can.
  - **C — B plus the coda in front of it** (`defold_codas_before`): the glyph immediately preceding a terminalized nasal is a folded coda that is really that syllable's onset. 考卷 `ㄎㄛㆻㄫ` → B `ㄎㄛㆻㆭ` → C `ㄎㄛㄍㆭ`.
- **Generator B eligibility is phonotactic, not positional**: a nasal can still be an onset when `<nasal><next glyph>` is a valid syllable (`phonetics::is_valid_syllable ∘ tps_to_tl`). This is what keeps 門 `ㄇㆭ` (mng), 兩 `ㄋㆭ` (nng), 龜毛's `ㄇㆦ` (moo) and 博雅's `ㄫㄚ` (nga) untouched. A "next char is vowel material" test would rewrite the two syllabic-`ng` nuclei — `ㆭ` is deliberately excluded from `is_tps_vowel_material` — and `ㄇㆭ` + `ㄋㆭ` carry ~1.4k production rows between them. Two-glyph window: TPS finals are not all one glyph (`ia` is `ㄧㄚ`, `ua` is `ㄨㄚ`), but a multi-glyph final BEGINS with a glyph that is itself a valid minimal final (`i` / `u`), so `<nasal><first body glyph>` already proves the nasal can open a syllable — `ㄇㄧ` (mi) answers for `ㄇㄧㄚ` (miâ 名).
- **Bare-nasal carve-out**: a canonical that is nothing BUT the nasal is skipped, so a bare `ㄇ` / `ㄫ` keeps producing zero span keys and `continuous.rs` keeps falling through to the partial-prefix branch that lists every m- / ng- initial word. Terminalizing it would emit a full-span key, make `keys.is_empty()` false, and drop that branch entirely.
- **Separator-aware**: generator A's eligibility check runs on `canonical` BEFORE the hyphen / TPS-space strip, requiring the coda to be immediately followed by vowel material — a coda before a TPS space (`ㄍㆤㆷ`␣`ㄧㄥ`) or a `-` 連字 is NOT de-folded. Generator C only ever looks at the IMMEDIATELY preceding glyph, so a space or 連字 between the coda and the nasal means they are not adjacent and the explicit boundary is respected: `ㄎㄛㆻ`␣`ㄫ` does NOT reconstruct 考卷 (§31). Generator B is deliberately the asymmetric case — a nasal before a separator is one the user has explicitly closed, which is exactly why it cannot be an onset, so B does fire there.
- **Scope (this round)**: success = the hidden word **appears in the candidate list**, NOT that it is slot-0 (both 考卷 and 毋是 in fact land at index 0/1 on production artifacts, but that is ranking's doing, not a guarantee). The walker uses the BASE lattice only, so an alternate never participates in whole-sentence synthesis; it can still sort to the visible top when the base reading yields no walker candidate (考卷 does), but it cannot displace one. TPS-only (`alternate_canonicals` returns an empty Vec for TL/POJ/English → zero behavior change there; the base path is byte-identical, pinned by the exact-key TL/POJ tests + `golden_fetch_at_pos`).
- **NOT generalized**: this is three fixed generators, not a general multi-site alternate enumeration. Still uncovered: A+B in the same alternate, mixed subsets of predecessor codas across several sites, >1 A-site buffers, and the `ㄧ`+`ㄫ` `ㄥ`/`ㆭ` dual reading.
...
**Engine sites**: `phonetics/src/tps_adjust.rs` (`defold_coda_to_initial` — inverse of `dual_final_form`; `nasal_final_form` — the narrow nasal window onto it), `phonetics/src/tps.rs` (`is_tps_vowel_material`), `phonetics/src/syllable.rs` (`is_valid_syllable`, generator B's eligibility oracle), `composing/src/shadow.rs` (`build_continuous_keys`, `alternate_canonicals`, `defold_single_coda`, `terminalize_stranded_nasals`, `nasal_can_open_syllable`, `defold_codas_before`, `lattice_from_canonical` shared tail), `composing/src/continuous.rs` (`assemble_candidates` calls `build_continuous_keys`), `composing/src/dispatch.rs` (`build_continuous_keys_with_inventory` — the full-key test seam; `build_keys_tl_with_inventory` stays the base-only seam).

**Tests**: `phonetics/src/tps_adjust.rs` (`defold_is_inverse_of_dual_final_form`, `nasal_final_form_covers_nasals_only_and_matches_dual_final_form`), `phonetics/src/tps.rs` (`is_tps_vowel_material_accepts_nuclei_rejects_coda_nasals`), `composing/src/shadow.rs` (`defold_single_coda_*`; `terminalize_stranded_nasals_*` — rewrites a stranded nasal, keeps `ㄇㆭ`/`ㄋㆭ`/`ㄍㄨㄇㆦ`/`ㄆㆦㆻㄫㄚ`, skips a bare nasal; `defold_codas_before_*`; `alternate_canonicals_*` — generator order + no de-fold across a separator), `composing/tests/build_keys_tps.rs` (`alternate_reading_*` — hermetic inventories carrying the BASE reading's syllables as well as the alternate's, so an alternate is proven to be an addition not a replacement). End-to-end (production artifacts) confirmed via `composing/tests/candidate_dump.rs`: `ㄎㄛㆻㄫ`→考卷 [0], `ㄇㄒㄧ`→毋是 [1], `ㄍㆤㆷㄧㄥ`→雞胸, `ㆦㆴㆤㆷ`→烏白, `ㄅㄠㆷㆦㆻ`→包袱; controls unchanged — `ㄆㆦㆻㄫㄚ`→博雅 [0], `ㄇㆭ`→門 [0], `ㄋㆭ`→兩 [0], `ㄍㄨㄇㆦ`→龜毛, `ㄍㄠㄉㄞ`→交代, `ㄎㄛㆻ`␣`ㄫ`→no 考卷, bare `ㄇ`/`ㄫ`→partial-prefix list intact. Cross-platform device acceptance: **S21** (part A) + **S23** (parts B/C), `.claude/rules/taigi-incidents.md` § Qualitative perf gate.

**Known follow-up (USER-gated)**: (1) librime ambiguity-penalty (score multiplier) on an alternate candidate; (2) ranking cohesion (whole-word > both-high-freq single-char split; McBopomofo epsilon-boost); (3) alternates participating in slot-0; (4) the uncovered generator shapes listed under **NOT generalized** above; (5) the TPS spacebar's missing tone-1/4 pinning (the same 2026-06-22 bug report's other half — the space is a separator with no tone semantics and the `tps:` FST namespace cannot express "tone 1 only", so it is its own round).
