...
**Tests**: iOS `SettingsKeyTests.swift` and Android `DisplayLanguageTest.kt` pin the production roster, exact selectable roster, `system` round-trip, unknown-tag → Hanji fallback, and production `tailo`/`poj` resolution. Python `tools/i18n/test_i18n.py` pins `PRODUCTION_LANGUAGES == ("hanji","en","ja","tailo","poj")` + missing-`tailo`/missing-`poj` rejection (the production-completeness gate enforces the `tailo`/`poj` pair — no separate lockstep gate). Each side pins its own literals; the `CROSS-PLATFORM INVARIANT` source comments + same-PR discipline are the drift guard (no runtime loads both platforms). The picker UI + cross-process live-switch are dogfood-pinned.
