...
**Tests**: `composing/src/dispatch.rs` (`literal_roman_candidate_*` — gate matrix + WYSIWYG + Tailo + `guá` identity fold) + golden `composing/tests/golden/fetch_at_pos.golden` (`tl_numeric_single :: tsua2` now leads with the `tsuá` Tailo row). Cross-platform device acceptance: **S22** (`.claude/rules/taigi-incidents.md` § Qualitative perf gate).
