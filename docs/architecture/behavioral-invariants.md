...
**And it draws a faint fill of its own**: the literal cell is what the user is currently typing, not a candidate the engine offered, so it is tinted to read as a different KIND of row (USER 2026-09-09) — macOS `NSColor.tertiarySystemFill` (a system fill, dynamic per appearance, resolved inside the panel's own `effectiveAppearance`), Windows twice the alpha of WinUI's `SubtleFillColorSecondary` (`Theme.literal_fill`, `None` under a high-contrast scheme, which is chosen to be free of decorative cards). Both are one step up from where they started on 2026-09-09 — the faintest fill did not read over the panel background (USER 「背景顏色稍微強調一點」). The literal cell's text also centres across the WHOLE cell under a stacked arrangement — it names no key, so it does not step around the key column the other cells align on (macOS `CandidateItemView.updateStackedTextArea`, Windows `draw_cell`'s `centres_across_cell`); the inline arrangement keeps the column, since the vertical list aligns every row's text on one x. Precedence is selection > literal tint > nothing, and only the SELECTION changes the text colours — so a highlighted literal cell (the fresh bar, which selects cell 0) is drawn like any other highlighted cell and the tint cannot read as a second selection.
