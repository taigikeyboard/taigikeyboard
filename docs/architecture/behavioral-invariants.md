...
- Android binding: `InputMethodService.onUpdateSelection` delivers `candidatesStart == -1 && candidatesEnd == -1` when the host has no composing region — but that report can be **older than the IME's own last `setComposingText`** (an app focus / draft-restore selection move emitted before the app processed the write; also `onStartInputView(restarting=true)` from an app-side `restartInput`). Treating every such report as current wiped the first key typed after an IME / app switch (USER report 2026-09-12). `TextInputManager.onUpdateSelection` therefore routes through `ComposingManager.onHostSelectionUpdate` → `reconcileWithHost(ic)`: while composing it re-reads `ic.getTextBeforeCursor(displayText.length)` — proxied to the editor after the earlier write, so a stale report sees the applied preedit — and keeps state on an exact match; on mismatch, short read, or `null` it calls `onExternalComposingRegionCleared()`, which zeroes state without touching `InputConnection`. **Keep policy is a text heuristic**: a host that dropped the span but kept the same text before the cursor — or a tap-away landing at the end of identical text elsewhere — is indistinguishable and is kept (the next key then inserts the whole preedit instead of replacing it, e.g. `tai` → `taitaik`); accepted. FlorisBoard's unmatched path goes further (re-derives the composing word and re-asserts the region via `setComposingRegion`); not adopted because Taigi raw input is not recoverable from document text. `bumpGeneration()` zeroes the mirror as well, so an input-context invalidation (mode switch with no `InputConnection`) can never leave a mirror the reconciliation would keep while the engine has already dropped the composition. `onStartInputView(restarting=true)` on a still-composing-eligible editor keeps the live `ComposingManager` (no rebuild, no host-region wipe, no generation bump) and runs the same reconciliation, re-fetching candidates when the composition survives. Closes the root cause of the A4-impl `commitComposition` fast/slow split (`composing-state-boundary.md` §11.10 divergence #3).
...
- Binding pin: Android `ComposingManagerHostReconciliationTest` — `reconcile_hostLostPreedit_clearsOnceWithoutWritingHost` (state zeroed, generation bumped once, IC untouched), `reconcile_hostStillHoldsPreedit_keepsStateAndGeneration` (stale report kept), `reconcile_nullRead_clears` / `reconcile_nullConnection_clears` (read failure = pre-existing clear), `reconcile_reentrantFromInsideWrite_seesNewPreedit` (synchronous host callback inside `setComposingText`), `reconcile_commitAThenStartB_lateCallbackForA_keepsB`, `reconcile_shortRead_clears` / `reconcile_unicodePreedit_keeps` (read-length policy).
