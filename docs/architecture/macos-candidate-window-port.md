...
| `indexLabels: String` + `candidateIndex(for:)` / `navigationIntent(...)` | Key classification stays in `ComposingKeyIntent` (the tested seam). Slots are a fixed nine addressed by `⌃1`…`⌃9`, and are NOT drawn — the index column is dropped entirely (§42, USER 2026-08-21: a modifier badge beside every candidate is visual noise, and the width it took is what the second script now uses). **Superseded**: the column came back in PR #604 (2026-08-24) once bare digits could select, drawing the key that actually fires; since 2026-08-28 the shipped keys are `q w d f z x v y ;` (`CandidateSlotKeySet`). See `behavioral-invariants.md` § two-script cell. |
