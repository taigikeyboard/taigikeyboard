...
| iOS / Android native store, an existing file the engine took over | Stamps unchanged (freq 2, assoc 6, learned 1, custom 6 / 10), legacy columns kept, so no migration re-runs and native writes still succeed. Compatible. Only the unreleased v9 `origin` / `learn_count` columns are gone, which no released build reads. |
...
**U9 — Transition compatibility is temporary.** Until P9, the engine honours the old row fields (`frequency_entries`, `custom_entries`, `learned_entries`, `PredictNext.user_rows`) and the persistence effects only when no store is open for that process. It never merges DB rows with proto rows (double weighting / double learning). Temporary, removed in P9.
...
| 2 | `feat(engine): user-data takeover and migration` | Feature (engine, `userdata` crate only) | U3 journal parameter (`JournalMode`) + reader wait under a rollback journal; per-file `UserDataPaths`; U7 takeover marker, future-version guard, pre-takeover copy, shape-detecting migrations + fixtures; U8 matrix. No platform caller yet: the proto ops and the process-wide handle moved to P3 with the dispatch wiring, where U11's fork is decided | In progress |
| 3 | `feat(engine): engine reads and writes user data` | Feature (engine) | `UserDataRequest` `Open` / `Reset` ops + process-wide handle; U11 dispatch fork; the engine's open runs the custom-dictionary re-derivation itself (today four desktop call sites must remember `rederive_search_keys_if_needed`, and the takeover of that store waits on them); `FetchAtPos` reads frequency / custom / learned itself (one call — brainstorm R5); `PredictNext` reads user rows (§24 order); engine persists association, learned phrase, frequency (setting-gated) on commit; U9 compat | Pending |
...
| 5 | `refactor(desktop): Windows + Linux use engine user data` | Platform switch | drop two-phase fetch, store traits, learner persistence; settings apps via the Rust API | Pending |
| 6 | `refactor(macos): use engine user data` | Platform switch | delete `Storage/` stores + two-phase fetch; settings pane via ops | Pending |
| S | iOS spike (no PR) | Spike | bundled rusqlite in the xcframework; open an App Group DB from the extension; binary size, extension memory, app + extension concurrent writes, suspension with no held lock | Pending |
| 7 | `refactor(ios): use engine user data` | Platform switch | app + extension open via ops (App Group dir, `DELETE`); delete repositories / schemas / migrators / services; `BackupService` → codec; §29 exclusion kept, extended to the `<file>.pre-engine` copies | Pending |
| 8 | `refactor(android): use engine user data` | Platform switch | `databases/` + `filesDir` layout via `Open`; delete services; `BackupService` → codec | Pending |
| 9 | `chore(engine): retire platform-pushed user rows` | Cleanup | reserve old proto fields / effects (U9); `migration-inventory.csv`, `data-artifacts-portability.md` (§4–8, D5), `system-overview.md:49`, behavioral-invariants updates | Pending |
