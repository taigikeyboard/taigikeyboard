...
| 7b | `refactor(ios): use engine user data` | Platform switch | app + extension open via ops (App Group dir, `DELETE`); delete repositories / schemas / migrators / services; `BackupService` → codec; §29 exclusion kept, extended to the `<file>.pre-engine` copies | Pending |
