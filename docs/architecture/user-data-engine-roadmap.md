...
**U8 — Downgrade matrix is decided in P2, not assumed.** An old app reopening a migrated file must not silently corrupt it. Traps: Android `SQLiteOpenHelper` throws on a higher version (`onDowngrade` default), and a `currentVersion >= schemaVersion` check (`NextWordService.kt:520-543`, `NextWordSchema.swift:27-42`) would skip migrations. Hence no "one number above every namespace" stamp; each store's stamp stays within what the previous release of that platform accepts, or the matrix records why downgrade is impossible (to verify: store-installed mobile apps cannot downgrade without deleting app data; desktop packages can be reinstalled older).
...
| 7 | `refactor(ios): use engine user data` | Platform switch | app + extension open via ops (App Group dir, `DELETE`); delete repositories / schemas / migrators / services; `BackupService` → codec; §29 exclusion kept | Pending |
