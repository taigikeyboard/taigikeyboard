...
**U9 — Transition compatibility is temporary.** Until P9, the engine honours the old row fields (`frequency_entries`, `custom_entries`, `learned_entries`, `PredictNext.user_rows`) and the persistence effects only when no store is open for that process. It never merges DB rows with proto rows (double weighting / double learning). Temporary, removed in P9. P9a: the engine no longer honours the `FetchAtPos` rows nor emits `Effect.PhraseLearned`.
...
| 9a | `refactor(engine): user rows are engine-internal` | Refactor (engine) | composing takes the user rows as Rust types (`composing::UserRows` on `Intent::FetchAtPos`, `EngineHandle::query`) and hands the phrase a final commit taught back beside the response (`Applied.learned`, `EngineHandle::handle_learning`) — no `Effect.PhraseLearned`; `decode_intent` ignores the wire rows; `ranking::build_frequency_map` (proto) gone, dispatch builds the `FrequencyMap` from the store rows; composing / lexicon / dispatch tests off the wire rows (test-side `Fetch`) | In progress |
| 9b | `refactor(engine): association rows and records are engine-internal` | Refactor (engine) | `PredictNext` user rows passed as an argument into the `FilterPredictions` build; nextword hands the bigrams it decides to record back beside the response — no `RecordAssociation` / `RecordCompoundAssociations` effects | Pending |
| 9c | `chore: reserve the retired user-data wire fields` | Cleanup | `reserved` FetchAtPos 2/4/7, PredictNext 2, Effect 11, NextWordEffect 3/4 (+ messages nothing else uses); bindings regenerated; platform no-op arms, desktop / macOS `AssociationSink`, iOS `NextWordAssociationPair` and their tests deleted (manager learning tests kept, association coverage in engine dispatch tests) | Pending |
| 9d | `docs: user data is the engine's` | Docs | `migration-inventory.csv`, `data-artifacts-portability.md` (§4–8, D5), `system-overview.md:49`, behavioral-invariants updates | Pending |
