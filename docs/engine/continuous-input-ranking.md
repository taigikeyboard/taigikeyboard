...
Fix-plan ordering: item 14 (visual unification) shipped 2026-05-13. Item 1 (doc reconcile) + Item 2 (`Phase::raw_input` accessor + invariant tests + §10.2 amendment for actual engine behavior) shipped on branch `v358-continuous-display-spec`. Item 3 (`Intent::CommitRaw` in `Phase::Continuous` commits `derived_display(pending)`, mid-commit preserves nailed segments) shipped 2026-05-14. Item 4 (Tap-0/Tap-N commit semantics — slot-0 cell removed from Continuous path, strict-required `displayText` sidechannel) shipped 2026-05-14. Item 5 (`CandidateMessage` `roman` + `hanji` proto fields + bridge decode + defensive `roman.isEmpty → displayText` fallback) is **in progress on branch `v358-item5-candidate-roman-hanji-proto`** at the time of writing. Remaining items 6-13 follow the dependency graph in the Codex transcript; next up = Item 6 (pin segmented dual-line rendering at platform UI).
