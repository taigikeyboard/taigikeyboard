...
| **R3** ✅ **PR #384** | 自訂詞 cross-mode lookup → **DONE**:USER 拍板側表全 parity + browser 納入。`custom_search_key(entry_id,family,form,key)` 側表 = {tl,poj,tps}×{num,notone,abbrev}+tps變體;engine `phonetics::custom_search`(2 proto op)+ `tps_num_from_tl`/`tps_abbrev_from_tl`;非破壞 migration(iOS schemaV 1→2 / Android v5→6 backfill);raw roman + custom_toneless_key lattice 路徑不動;legacy 欄 write-only 保留 rollback。Codex post-impl HIGH(TPS tone-8 U+02D9↔U+0307)已修+測。Pins §26 + S13 | engine `custom_search.rs`/`tps.rs`/proto + 兩平台 schema/query/bridge/call-site + 非破壞 migration | 中-高 | ✅ done(設計 fork 非兩案相當→write-side 明確優) |
...
| **R6** | SQLite hygiene(slim,Codex pre-impl 收斂):①移除冗餘 `idx_user_prev_word`(複合索引左前綴子集)+ assoc schema **4→5** 無條件 migration `DROP INDEX`(涵蓋 v3 直跳 v5);②freq/assoc record-path **停止吞錯**(helper throw/單一 boundary structured log + 自覺忽略,不上拋打字路徑;修 iOS double-swallow);③user-initiated bulk-clear(`clearAll*`/`deleteAll`)後 best-effort `VACUUM`(背景、finalize 後、非交易內、失敗只 log);④Fork-1 migration 後 `PRAGMA optimize`。**刻意不做**:integrity_check(無 surfacing=死碼)、post-prune VACUUM(bounded LRU 重用 freelist)、`auto_vacuum=INCREMENTAL`(既有 DB 需 full VACUUM)。**延後(named deferrals,未指定版本)**:(a) journal-mode flip — iOS 3 DB pin DELETE(App Group 跨進程)vs Android assoc DELETE/freq+custom 待**裝置 `PRAGMA journal_mode` 實測**(Android `SQLiteOpenHelper` WAL 非預設);單進程 Android 強制 DELETE 無收益且最難證明無尾延遲 → **不改 code,記為 intentional per-platform divergence**;(b) custom-save 靜默失敗 = Android log-only vs iOS throws 的 UX/data-loss divergence,surfacing 需 UI(非 fire-and-forget)→ 另議 | iOS+Android | 低 | ✅ auto |
