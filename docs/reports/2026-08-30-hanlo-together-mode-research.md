...
> **Update (USER, 2026-09-01, paraphrased)**: the setting is a **mode picker named 候選詞顯示**, not a boolean toggle — mode 1 **漢羅並排** = today's title/subtitle rendering (default); mode 2 **漢羅合用** = the mode researched here, **renamed from 漢羅齊出** (the name in the verbatim statement above); mode 3 **羅馬字** = added later the same day, researched in **Part II (§12-§20)**. Every 漢羅合用 mention below refers to what the original statement called 漢羅齊出.
...
3. **Wire**: `CandidateMessage` field 11 `bool is_correction` (display marking + platform-side UX decisions); `AppConfig` field 9 for the 候選詞顯示 mode (the engine must know the mode anyway if correction is mode-gated — and Part I §5/§10 already concluded a dedicated field beats overloading `is_translate_swapped`/`output_both_scripts`).
...
|---|---|---|
| Cell content | title/subtitle (並排) or one label (合用) | **single roman title, no subtitle** (R11) — the engine `roman` field (display form: POJ-rendered / recased), not `canonical_tl`. MIXED rows (`台BAR`) show their roman (`tâi-bà`). Needs a §42 carve-out (third arrangement) like Part I's. |
...
| Same-roman rows | distinct by hanji (subtitle/title) | visually identical roman-only cells → **display dedupe by `(rendered roman, span)`**, mirroring the TPS `(hanji, span)` display dedupe (`composing/src/continuous.rs:360`, §5 two-tier `behavioral-invariants.md:132-155`). Engine-side recommended (one implementation, 4 platforms) — needs the engine to know the mode → AppConfig field 9. Survivor = top-ranked row of the group (Q9′). |
| §34 literal | index 0; guard drops identical **bare-roman** dict rows only (`dispatch.rs:288-296`) | stays index 0. A hanji-bearing dict row whose roman equals the literal (`台/tâi` vs `tâi`) renders identically now — the `(roman, span)` display dedupe collapses it; literal wins slot 0 as today. |
| 文/A swap key | visible | no hanji anywhere → hidden or inert, same option set as Part I Q3 → Q11. |
...
| `continuous_word_space` / auto-space / S10 | predicate `!swapped ‖ both` | roman-ish = **true** unconditionally; single-source via the AppConfig mode field. |
| nextword | `shape_prediction` text/subtitle; swapped gates recording | send swapped=false semantics (Enter-raw recording stays, `decide.rs:93`); prediction cells render roman-only like any cell (`subtitle` simply unused). |
| macOS Space = alternate script (#610) | reads the cell's annotation | roman-only cells have no annotation → `alternateText` returns nil → Space already `.ignored` (`CandidateDocumentText.swift:70-79`, `ComposingManager.swift:371-374`). **Zero work; verify in dogfood.** |
...
- Same-roman different-hanji rows (`tsia̍h` 食/𤆬) become **visually identical roman-only cells** — the one new duplicate class this mode creates. Engine identity dedupe `(roman, hanji, span)` correctly keeps both (#7 — different words); the fix is a **display-tier** `(rendered roman, span)` dedupe exactly like TPS's `(hanji, span)` one (§17 row 3). Same-roman different-**tone** rows (tîng vs tāng) render differently and correctly stay separate.
- Stores: no schema/key change. Commit in this mode = today's roman-first commit path verbatim (same `display_text` / `canonical_tl` / `association_tl` sidechannels), so 詞頻/詞關聯 behave exactly as roman-first mode does today. The only nuance: a collapsed group learns under its **surviving** row's identity; hidden rows don't learn (acceptable — same as any not-shown candidate) → Q9′ confirms survivor rule.
- 自訂詞: untouched; custom rank-0 still wins its `(roman, hanji, span)` slot before display dedupe.
...
| # | Question | Why it matters | Options seen |
|---|---|---|---|
| Q9′ | Same-roman collapse survivor rule: top-ranked row of the `(rendered roman, span)` group wins and its identity is what a commit learns — confirm, or prefer another rule? | §18 learning nuance; likely an engineering detail unless USER objects | top-ranked survivor (recommended) |
| Q11 | 文/A key hidden or inert (Part I Q3 rerun) | same trade-offs: iOS layouts hardcode the key in 9 rows | hidden · inert |
...
| Layer | Files | Nature |
|---|---|---|
| engine | `envelope.proto` AppConfig field 9 (display mode) + `derived.rs` mirror + proto regen; `(rendered roman, span)` display dedupe arm beside the TPS one (`composing/src/continuous.rs:294-299,360` area); tests | ~4 files; `make build` |
| iOS | `CandidateCellHelper` roman-only arm (title = roman, subtitle = nil — `CandidateButtonView`/grid cell need no change if subtitle is nil-safe); settings picker (3rd value); preview mocks | ~4-5 files |
| Android | strip + overlay chooser roman-only arm (`SmartbarCandidateStrip.kt:198-232`, `CandidateOverlayContent.kt:358-364`); settings picker | ~4-5 files |
| macOS | `CandidateCellContent` roman-only arm (annotation = nil; #610 Space then auto-ignores); settings picker; `AutoSpacePolicy` mode predicate | ~4 files |
| Windows | candidate window cell roman-only arm + settings (blind surface — verify against a Windows build) | ~3 files |
...
| docs | `INVARIANT_*` §§: roman-only cell arrangement + `(roman, span)` display dedupe (mirror of TPS two-tier); §42 carve-out; fix `khiin-reference.md:128` (no fuzzy tier exists); `mainstream-ime-comparison.md` correction topic row when the future 校正 proposal lands | |
...
- Same-roman collapse: `tsia̍h` homophones (食/𤆬) → **one** `tsia̍h` cell (vs two rows in 並排); 重's two readings tîng / tāng → **two** cells (different roman, #7 intact); §34 literal `tâi` next to dict `台/tâi` → one `tâi` cell, literal keeps slot 0.
...
- Windows: candidate window shows roman-only cells; behaviour otherwise unchanged.
