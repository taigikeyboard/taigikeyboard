...
**與暫停中 3 bug 的關係**:Bug 2 + §1 `taiuantaigi` 由 **S2 吞掉(✅ DONE,branch `lattice-s2-walker`)**(lattice 自然產出全 buffer 最佳路徑,Bug 2 合成羅馬字成為 walker 在無漢字路徑時的 roman 最佳路徑,**非特例 fallback**,符合 `feedback_no_redundant_fallback.md`;`walker::tests` `no_dict_path_is_per_syllable_roman` / `picks_high_frequency_phrase_path_over_single_chars` 已覆蓋邏輯)。**Bug 1(swap commit)、Bug 3(preedit 生命週期)與斷詞正交**,已各自獨立修並 merged(Bug 1 #281、Bug 3 Model B #283)。
