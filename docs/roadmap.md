...
**Decision**: 2026-05-15 user 採納策略。**2026-05-16 USER DIRECTIVE**:主流對齊斷詞 = **v3.5.8 must-solve**(覆寫先前「排程未定/不掛版本號/前置=release prep 後」)。**Status**: 切片實作中 — **S1 = DONE**(branch `lattice-s1-builder`,嚴格行為中性 lattice builder,Codex pre+post sandwich PASS);**S2 = DONE**(branch `lattice-s2-walker`,引擎-only 全句 walker + cost + span-aware dedupe + `best_candidate_for_key` seam,Codex pre-impl 設計裁定 + post-impl sandwich:BLOCK→修 P1 trailing-hyphen + P3 doc→re-review **SHIP**);S3 pending。**Source-of-truth**:本節 + [`docs/engine/continuous-input-ranking.md`](engine/continuous-input-ranking.md) §1–§9 (gap 證據鏈) + §7 G1/G2/G3 (goal axes)。release scope/timing 由 user 明示(`feedback_no_unilateral_release_scope`)。
