# v3.5.8 連續輸入 — Plan & Archive

> **Type**: Release archive (SHIPPED)
> **Keywords**: `archive`, `v3.5.8`, `continuous-input`, `連續輸入`, `Phase 0-9`, `整句 lattice + walker`
> **Status**: **SHIPPED** — release commit `61df3028` on 2026-05-20
> **Authoring history**: First drafted 2026-05-10 (Phase 0/1/1b/2/3 merged); evolved through Phase 9 + 整句 lattice + walker S1-S9 + continuous-compound-hyphen fix; archived in this location 2026-05-24 per [B2 docs-only round](../../roadmap.md#released-versions-index).
> **Source-of-truth note**: this file was previously `docs/roadmap.md`. Inbound code citations of the form `§ Phase N` / `§ Phase 9 R2 Q3.a` / `§ 整句 lattice + walker S<N>` resolve here. Inbound line-number citations (`docs/roadmap.md:NNN`) are stale post-move and should be migrated to section-name refs in a follow-up code-cite round (Slice B).
> **Release notes**: see [`changelog/v3.5.8.md`](../../../changelog/v3.5.8.md).
...
- ~~TPS tone-1 (無調號) 隱式邊界不支援~~ **RESOLVED — v3.5.8 Phase 9 Item 7**:`tps::valid_span_endings` 加 next-initial-seen rule (已吃韻核後見新聲母 / 輸入結尾仍有未結第 1 聲 → 補切點),`build_keys_tps` 接受無數字 toneless 片段。打 `ㄉㄞㆣㄧ` 等無調號注音可 surface 候選
...
For each RawCandidate produced by fetch_candidates_for_keys:
  ...
  sort_key = (
      coverage_kind,    // asc:  Item 10 — full-syllable(0) 先於 partial-prefix(1)
      tier,             // asc:  Tier 0 全 buffer 優先
      recency_rank,     // asc:  同 tier 內,recent 先
      -adjusted_score,  // desc: freq × syll × boost
      -freq,            // desc: 原 freq 二次 tie-break
      -coverage_bytes,  // desc: S8 — 已降為 score/freq 之後的弱 tiebreak(原在 tier 之後)
...
// v3.5.8 整句 lattice + walker S8:`-coverage_bytes` 從 tier 後(dim 2)
// 降到 score/freq 後(dim 6)。slot-0 walker 已負責整句片語優先,tier 內
// 「長覆蓋優先」只會把使用者要逐段點選的單音節首段壓底(dogfood
// `guaikingkahuekhoo`→「我」排在雙音節之後)。現只在 score+freq 同分時
// 才以覆蓋長度分 — 對齊 librime per-segment menu(`script_translator.cc`
// `kNumExactMatchOnTop`:保留多長度但不讓長碼蓋短嚴格匹配)。
```
...
| 9.4a — TPS tone-1 next-initial-seen rule | ~50-100 Rust | 9.4b | **Yes**(coverage)| **In progress (branch `v358-item7-tps-tone1-rule`)** |
...
**Active PR pointer**:current round = **Phase 9 fix-plan Item 13 — retire platform lexicon fallback (v3.5.8 capstone)** **In progress (branch `v358-item13-retire-platform-fallback`)**(deletion-heavy 兩端,engine 成唯一候選來源;iOS 整檔刪 `LexiconService.swift`(273)+ `AutocompleteInputClassifier.swift` + `LexiconServiceHanziGuardTests.swift` + `CompositionRoot.lexiconService`,`AutocompleteService` 收斂 engine-only(無 await → 移除 stale guard);Android `LexiconService.kt` method 級刪 `search()`+3 helper(class 留給 Tab3 `searchWithSources`/`searchByHanzi`)+ 刪 `AutocompleteInputClassifier.kt` / `LexiconServiceHanziGuardTest.kt`,`TaigiAutocompleteService` 收斂 + ctor slim + `CandidateUpdateCoordinator` mode-cache/dead-arg 清理;平台 D-8 guard 隨 retire 移除(engine Item 11 已 own,`behavioral-invariants.md` §14 re-point engine);spec §15.4 同 PR 修正 iOS/Android 不對稱;`lexicon.proto` ClassifyInput 註解 refresh + regen comment-only;新增 §15.6 `platform_autocomplete_no_lexicon_branch` 兩端 test。Item 13 收尾即 v3.5.8 feature-complete(之後 = release prep / dogfood / cut tag,非新 feature 輪)。** Items 1/2/14 (spec foundation) shipped in PR #267;Item 3 shipped in PR #268 (squash `29b5d437`);Item 4 shipped in PR #269 (squash `9a34d09a`);Item 5 shipped in PR #270 (squash `1c18804c`)— `(roman, hanji)` ranker dedupe **deferred to Item 12**;Item 6 (pin segmented dual-line rendering at platform UI) shipped in PR #271 (squash `3a33245b`);Item 8 (hyphenated TL shadow buffer) shipped in PR #272 (squash `4a41c74c`,dogfood ✓ 2026-05-14);Item 9 (POJ diacritic canonicalize) shipped in PR #273 (squash `cf48bbae`,dogfood ✓ 2026-05-15)。**Sequence pivot 2026-05-14**:Item 7 (TPS tone-1) 延後到 capstone 前最後一輪,序列 = 9 → 10 → 11 → 12 → 7 → 13(全 merged / Item 13 in progress)per [`continuous-commit-and-display.md` §10.10](engine/continuous-commit-and-display.md)。Phase 0 merged in PR #248,Phase 1 merged in PR #249,Phase 1b N/A merged in PR #250 (squash `2c826b96`),Phase 2 merged in PR #251 (squash `f4c2e52f`),Phase 3 merged in PR #252 (squash `2f7feac1`),Phase 4 merged in PR #253 (squash `a69bfc75`),Phase 5 merged in PR #254 (squash `cf813af4`),Phase 6 merged in PR #255 (squash `c6f2ca42`),Phase 7A merged in PR #256 (squash `8c431af6`),Phase 7B merged in PR #257 (squash `65c2120c`),Phase 8 merged in PR #258 (squash `9fed869b`),Phase 9.1 merged in PR #262 (squash `86dd6253`),Phase 9.2 merged in PR #263 (squash `f5f2a3b2`),Phase 9.3a merged in PR #264 (squash `60601681`),Phase 9.3b merged in PR #265 (squash `a5d2fcdd`),Phase 9.3c merged in PR #266 (squash `0c0aa2ec`),Phase 9 spec foundation (items 1+2+14) merged in PR #267 (squash `2b65bd78`),Phase 9 Item 3 merged in PR #268 (squash `29b5d437`),Phase 9 Item 4 merged in PR #269 (squash `9a34d09a`),Phase 9 Item 5 merged in PR #270 (squash `1c18804c`),Phase 9 Item 6 merged in PR #271 (squash `3a33245b`),Phase 9 Item 8 merged in PR #272 (squash `4a41c74c`),Phase 9 Item 9 merged in PR #273 (squash `cf48bbae`),Phase 9 Item 10 merged in PR #275 (squash `71107ae7`),Phase 9 Item 11 merged in PR #276 (squash `2be7a60b`),Phase 9 Item 12 merged in PR #277 (squash `707853c0`),Phase 9 Item 7 merged in PR #278 (squash `9f7fda55`);Phase 9 Item 13 (capstone — retire platform lexicon fallback) in progress on branch `v358-item13-retire-platform-fallback`。
...
**Decision**: 2026-05-15 user 採納策略。**2026-05-16 USER DIRECTIVE**:主流對齊斷詞 = **v3.5.8 must-solve**(覆寫先前「排程未定/不掛版本號/前置=release prep 後」)。**Status**: 切片實作中 — **S1 = DONE**(branch `lattice-s1-builder`,嚴格行為中性 lattice builder,Codex pre+post sandwich PASS);**S2 = DONE**(branch `lattice-s2-walker`,引擎-only 全句 walker + cost + span-aware dedupe + `best_candidate_for_key` seam,Codex pre-impl 設計裁定 + post-impl sandwich:BLOCK→修 P1 trailing-hyphen + P3 doc→re-review **SHIP**);**S3 = DONE & MERGED**(squash → main **`5bcb9bc9`** #286,branch 已刪,main pulled;引擎-only:`ranking::decayed_user_weight_delta` librime `formula_d` 牆鐘版時間衰減 + `cost::edge_score` syllable-aware user-weight + McBopomofo epsilon-boost,接 Gap B → G2;Codex pre-impl Q4 裁定:cap-before-decay + single-syllable 阻尼為 BLOCK 條件,實作遵守;post-impl sandwich:SHIP-WITH-FIXES 1×P2 test-coverage→補非零齡飽和測試→re-review **SHIP**;`/codex-pr-review` user 明示 SKIP)。**Source-of-truth**:本節 + [`docs/engine/continuous-input-ranking.md`](engine/continuous-input-ranking.md) §1–§9 (gap 證據鏈) + §7 G1/G2/G3 (goal axes)。release scope/timing 由 user 明示(`feedback_no_unilateral_release_scope`)。

> **⚠️ S2 設計決定(Codex pre-impl Q1c = option ii,2026-05-16 — 需 R4 dogfood 驗收,非靜默移除)**:原 S1 設計修正把「start-aware `CommitContinuous`(帶 span start)+ 獨立可點內段候選」移入 S2。Codex pre-impl 裁定:Model B(§10.3/§10.4)commit 是 **forward-only** `pending[..consumed_bytes]`,獨立可點內段無 Model-B-一致的 commit;加 `consumed_start` 會破壞 forward-only 不變式且擴成 proto/api/transition/雙平台 binding 大改。**採 (ii):S2 = 引擎-only** — walker 內部吃內段邊、輸出單一全 buffer 最佳路徑 explicit prepend 到 slot 0;user-facing commit span 維持 `(0,end)`;`CommitContinuous` 不動。Finding 3 的「台語」於 nail「臺灣」前綴後以**下一步 path-step** 可達(非首列獨立可點內段)。**此為實作設計裁定,非 release scope 決定**:S2 仍 ship v3.5.8;若 R4 dogfood 顯示「下一步可達」對 Finding 3 不足,再開後續 round 評估 `consumed_start`(屆時 user 定 scope)。Codex transcripts:`/tmp/lattice-s2-preimpl-out.txt`、`/tmp/lattice-s2-postimpl-out.txt`(BLOCK)、`/tmp/lattice-s2-rereview-out.txt`(SHIP)。
...
**Grounded 現況**(file:line,**S2 後更新**):切音節 `engine/composing/src/syllabifier/tl.rs:49-84`(BFS 回傳 ending **集合**,純函式,S1/S2 未改);`engine/composing/src/lattice/{mod,builder}.rs`(S1 `build_lattice` 多起點完整 DAG)+ **S2 新增 `lattice/{walker,cost}.rs`**(`walk_best` 單趟 relaxation + `edge_score` khiin-family log-prob;walker 純 shadow-space);建 key/walker `engine/composing/src/dispatch.rs`(`build_shadow_lattice` 共用 shadow+DAG;`build_keys_tl_with_inventory` 仍只發 **`(0,end)`** 左錨投影 = 行為中性;**S2 `fetch_walker_slot0`** 持 LexiconHandle、注入 edge-provider、`synth_consumed_span` trailing-hyphen guard、explicit prepend slot 0);取候選 `engine/lexicon/src/continuous.rs`(span-local `lookup_exact` + **S2 `best_candidate_for_key` 單 key 最佳候選 seam** + dedupe key 改 `(roman,hanji,consumed_span)`);排序 `engine/ranking/src/score.rs`(span-local 路徑 `freq×(1+0.1·max(0,syll−1))×user_freq_boost`;**S3 新增** `decayed_user_weight_delta` = librime `formula_d` 牆鐘版時間衰減純函式 + `USER_WEIGHT_DECAY_TAU_MS`=30 天 τ,dogfood 可調 14–90 天);walker edge cost `engine/composing/src/lattice/cost.rs`(**S3**:`edge_score` 加 syllable-aware user-weight 乘性項 + `WALKER_PHRASE_EPSILON`=0.001 多音節 epsilon-boost + `WALKER_SINGLE_SYLLABLE_USER_DELTA_SCALE`=0.0 單音節阻尼;接點 `dispatch::fetch_walker_slot0` 持 `freq_map`+`now_ms` 算 per-edge delta,`best_candidate_for_key`/`record_to_candidate` 不動)。**walker 已上(S2)**;**Gap B 已關(S3,branch `lattice-s3-userfreq`)** → 收斂 G2。
...
| **S2 ✅DONE** | 全句 walker:`edge_score` khiin-family log-prob + 長度正規化,`walk_best` 單趟 relaxation,輸出單一全 buffer 最佳路徑候選 **explicit prepend slot 0**(Codex Q1 — SortKey 無法保證 slot 0);**span-aware `(roman,hanji,consumed_span)` dedupe**(Codex Q1d,Item-12 custom-vs-dict.bin 仍 `(0,raw_len)` 碰撞);`best_candidate_for_key` 重用 `record_to_candidate`(Codex Q1b seam);`synth_consumed_span` trailing-hyphen guard(Codex post-impl P1)。**收斂 G1 + 吞 Bug 2(`taiuantai`→`tai uan tai`)+ §1 `taiuantaigi`→臺灣台語**(無漢字路徑時 walker 自然產出 roman 最佳路徑,非特例 fallback)。**Codex Q1c=(ii) 重塑(見上方 ⚠️)**:start-aware commit / 獨立可點內段 **未做**(破 Model B forward-only + 跨平台大改);Finding 3 以 nail 前綴後**下一步 path-step** 可達,待 R4 dogfood 驗收。`CommitContinuous` 不動 | S1 |
| **S3 ✅DONE & MERGED**(main `5bcb9bc9` #286) | 關 Gap B:`user_frequency.db` 動態權重經既有 `FrequencyMap` plumb 接進 walker edge cost。`ranking::decayed_user_weight_delta`(librime `formula_d` 牆鐘版:`delta=(user_freq_boost(count)−1)×exp(−age/τ)`,**cap-before-decay** 防 stale 單一 entry 數月壓制 = Codex Q4a/Q4c BLOCK 條件;τ=30 天,dogfood 14–90 天);`cost::edge_score` 乘性 user-weight + McBopomofo `WALKER_PHRASE_EPSILON`=0.001 多音節 tie-band nudge + `WALKER_SINGLE_SYLLABLE_USER_DELTA_SCALE`=0.0 單音節阻尼(防熱門單字如「的」壓整句 = Q4c BLOCK)。Codex Q4d seam:`EdgeChoice.user_weight_delta`,walker 仍純 shadow-space,`best_candidate_for_key`/`record_to_candidate` 不動(record-選擇 vs path-目標 正交不重複計)。S2 no-dict tie lever 中性契約保持。收斂 G2。Codex pre+post sandwich SHIP(post 1×P2 test-coverage→補→re-review SHIP) | S2 |
...
| **S5 ✅DONE & MERGED**(squash → main `3830af39` #287) | **Cost-model 修正(must-solve)**:S2/S3 的 `edge_score` 把 khiin 公式的 `ln(1/p)` 正規化分母 + minimize 拿掉,變成全正、無正規化、`1+` 地板的 **maximize Σ** → 結構性獎勵過度切分(dogfood:`taiuan`→`乾伊有俺`)。S5 忠實移植 khiin `segmenter.rs:82-93`:`p=(1+freq)/CORPUS_TOTAL_FREQ`、`cost=ln(1/p)/len^0.2·n_syls^0.2`、walker **MINIMIZE Σ cost**。`EdgeChoice` 加 `dict_hit`+`toneless_len`;no-dict 改 `fetch_walker_slot0` **顯式 carve-out**(整路徑零 dict-hit → greedy-longest syllabify roman,不進 cost);S3 user-weight 改 log-space `cost −= ln_1p(applied_delta)`(Q4c 單音節阻尼保留);丟 `WALKER_PHRASE_EPSILON`(band-aid)+ walker "more edges wins" tiebreak。Codex pre-impl PASS(6Q 裁定,2 BLOCK:dict_hit 不可用 hanji/freq 推、需 toneless_len)。Goal:`taiuan`→台灣、`taiuantaigi`→台灣台語。**engine-only,無 proto/平台改動** | S3 |
| **S7 ✅DONE**(branch `lattice-oov-edge-cost-fix`) | **OOV-cost 修正(S5-followup,dogfood must-solve)**:S5 忠實移植 khiin 的**字典詞**分支(`p=(1+freq)/CORPUS`)卻把 khiin 未知詞分支 `p=1e-5/10^word_len`(`references/khiin-rs/khiin/src/data/segmenter.rs:83`,機率隨長度衰減)換成**長度無關**的 `1/CORPUS`,且 `fetch_walker_slot0` no-dict edge `syllable_count` **寫死 1**。→ 整段 OOV blob 只付一次 `ln(CORPUS)` 稅又吃 `÷len^0.2` 折扣 → 比正確字典切分還便宜 → walker 選 blob、`any_dict=false`、carve-out 吐純羅馬字(dogfood `taiuanta`→`tai uan ta`、`taiuantai`→`tai uan tai`,2026-05-18 真實字典重現,score −10.80 = `ln(CORPUS)/8^0.2` 精確吻合)。S5 module doc 只推理「**全** OOV buffer」(carve-out 渲染),漏了 mixed case(存在字典路徑卻被便宜 OOV blob 蓋過)。修=`cost::UNKNOWN_SYLLABLE_DECAY=10.0`(khiin `/10^word_len` 的語料正規化版,改 key 在**實際音節數**:`p_oov=(1/CORPUS)/10^syllable_count`)+ `edge_cost` 加 `dict_hit` 參數(OOV 分支**只**由 `dict_hit==false` 選,絕不從 `freq==0` 推 — 真實字典詞可 freq 0、custom 用 proxy freq,Codex Q2)+ no-dict edge `syllable_count` 改取真實段數(byte-identical 於 custom 分支,Codex Q3)+ compile-time guard `UNKNOWN_SYLLABLE_DECAY>1.0`。**驗證**(真實字典):`taiuanta`→`台員乾`、`taiuantai`→`台員台`、`taiuantaigi`→`台灣台語`(不變)。**engine-only 零平台 diff,無 toggle**。Codex pre-impl PASS(Q1/Q5 BLOCK 已解:不宣稱「OOV 永遠輸任何字典路徑」全域保證,只 pin「輸給預期最佳字典路徑」+ hermetic walker 測;非全域 lexicographic 規則)。Codex post-impl sandwich + `/codex-pr-review` 收尾 | S5 |
| **S6 ✅DONE**(branch `lattice-s6-custom-slot0`,PR #289) | **自訂詞庫進整句最佳路徑(audit 缺口 1)**:`custom_dictionary.db` 已由 Item 12 plumb 成 engine `CustomEntry`,但原本只進 span-local `(0,raw_len)` 候選 —— `fetch_walker_slot0` **無 `custom` 參數**,edge provider 只打 `dict.bin`(`continuous.rs:736`),custom 永遠進不了 slot-0 walker 路徑。S6 = thread `&[CustomEntry]` 進 `fetch_walker_slot0`;新 `custom_toneless_key`(**重用 edge 自己的 `canonicalize_poj_shadow`→`build_hyphen_shadow`→`strip_ascii_tone_digits` pipeline → byte-identical match**,Codex pre-impl S6 Q2 BLOCK;roman-only,非 TL-shaped→None);per-fetch **first-wins** `HashMap<toneless,&CustomEntry>`(Codex Q6);edge toneless 命中 custom → custom 內容**優先於** `best_candidate_for_key` dict 結果(= span-local source-rank-0 勝去重語意,Codex Q3)。cost rule = **(a) effective-freq proxy `cost::CUSTOM_EFFECTIVE_FREQ=2_000`**(Codex pre-impl S6 Q1 BLOCK:**非** cost floor,custom 仍付 ln(CORPUS) 正規化稅;值 gate 於 dictionary.csv 百分位);`EdgeChoice.is_custom`→ path 含 custom edge 時傳到 synth `RawCandidate.is_custom`(Codex Q4);custom edge `dict_hit:true`(lexicon-backed,all-OOV carve-out 不觸發,Codex Q7 doc-guard);#288 per-edge recase 套 custom roman(Codex Q5)。**G1a only**:整串==custom 詞 → slot 0;G1b mid-sentence 僅 custom roman 天然對齊音節邊界時免費附帶,**不加非-syllabifier edge**。engine-only 零平台 diff。**Codex sandwich:pre-impl PASS(2 BLOCK 已解)+ post-impl SHIP-WITH-FIXES(僅 1×P3 = 本 row 過期 → 已修;無 BLOCK/P1/P2)**;`cargo test --workspace` 綠、clippy 0、touched-files rustfmt-clean。剩 `/codex-pr-review` + dogfood(需 `make build` 重建引擎二進位後驗) | S5 |
| **S9 ✅DONE**(branch `fix/walker-oov-blob-long-sentence`,RC0) | **長句 OOV blob 修正(dogfood RC0,S7-followup)**:`ginalangtsiahpngbesai`(囡仔人食飯袂使,完全字典可覆蓋)slot-0 = 純羅馬字 `gin a lang tsiah png be sai`,門檻**恰 6 音節**(5 音節 ✅ 漢字 / 6+ ❌ blob;真實字典重現)。**根因**:khiin **兩個被 S5/S7 混淆的機制** —(1)`segmenter.rs:75-92` 已索引詞 `p<=0` 的語料缺口下限 `p=1e-5/10^word_len`(ln 尺度);(2)`segment_min_cost:198-206` 真・未覆蓋 span 不用任何機率,DP 每前進一字元付 `BIG=1e10`(`segmenter.rs:29`),整段累加 `BIG/字元` → khiin 有字典路徑就幾乎不吐 OOV。S5 用機制(1)平滑機率定價真・OOV span → 整段 blob 一次吃 `÷word_len^0.2` 折扣,贏過逐邊累 `ln(CORPUS/freq)` toll 的字典路徑(>6 邊翻車);S7 `UNKNOWN_SYLLABLE_DECAY` 只調斜率沒解混淆(`taiuanta`/`taiuantai` 只是剛好在 6 邊門檻下)。**修(engine-only,零 proto/平台/Intent/Model-B,無 toggle)**:de-conflate — `edge_cost` OOV 分支改 khiin 機制(2):`OOV_PER_CHAR_PENALTY=1e10`(khiin 字面 `BIG`)× `toneless_len`,**無 bias、無 user 折減**;字典分支維持完整 khiin 公式不動;移除 `UNKNOWN_SYLLABLE_DECAY` + 其 guard,新增 `assert!(OOV_PER_CHAR_PENALTY>=1e5)` compile-guard;`span_min_syllable_count` 保留(OOV cost 不再用 syllable_count,但仍餵 synth 音節 sum metadata,Codex RC0 Q3)。「OOV 輸給任何字典可覆蓋路徑」任何長度恆成立 = **成本性質**(khiin 自己的 BIG 常數)非 dict_hit lexicographic 短路(對齊 Codex S7 Q1 / RC0 Q2)。**驗證**(真實字典):`ginalangtsiahpngbesai`→囡仔人食飯袂使、6 音節亦漢字;`taiuan`/`taiuanta`/`taiuantai`/`taiuantaigi` 仍漢字(無 S5/S7 regression;`台員` vs `台灣` = 正交 record-selection,非本 bug)。**最佳實踐對齊**:khiin `segment_min_cost:198-206` `BIG` per-char(第一手讀原始碼,非記憶)。Codex pre-impl **No BLOCK**(7Q,Design A 確認:per-char `BIG*toneless_len`、無 S7 Q1 違反、移除 DECAY、留 BIG=1e10、留 OOV syllable 為 metadata)。新 cost/walker hermetic RC0 invariant 測 + 既有 OOV 測 reframe;`cargo test --workspace` 綠、clippy 0、touched-files fmt-clean。剩 Codex post-impl sandwich + `/codex-pr-review` + dogfood(需 `make build` 重建引擎二進位後驗) | S5/S7 |
| **S8 ✅DONE**(branch `continuous-sortkey-coverage-demote`) | **SortKey 覆蓋維降級(dogfood must-solve)**:dogfood `guaikingkahuekhoo`(guá+í-king+kā+hue-khoo)第一段想點單音節「我」(Guá)卻排在雙音節候選之後,逐段點選很慢。**根因**:slots 1..n span-local `SortKey`(`continuous.rs`,連續路徑專用,非 wire)第 3 維 `neg_coverage`(長覆蓋優先)排在 `neg_score`(第 5)/`neg_freq`(第 6)**之上** → 整 buffer 無詞蓋滿 → 全 tier 1 → tier 內任何較長左錨前綴(含 4-byte 單音節)無視頻率壓過 3-byte「我」。`neg_coverage` desc 是 **walker 上線前**的 Gap-A 修法(spec §1.1);整句優先已由 **slot-0 walker** 接手(§7.3「G1 closed by S2」),此維變冗餘且主動埋葬單音節首段。**修(engine-only,零 proto/平台/Intent/Model-B,無 toggle)**:`neg_coverage` 自 dim 3 → dim 6(降到 `neg_score`/`neg_freq` 之下,`source_rank` 之上),`coverage_kind`(dim 1)/`tier`(dim 2)不動 → 覆蓋僅在 score+freq 同分時為弱 tiebreak。**最佳實踐對齊**:librime per-segment menu(`references/librime/src/rime/gear/script_translator.cc::PrepareCandidate` `kSentence` 置頂〔= 我們 slot-0 walker〕+ `kNumExactMatchOnTop=1`「長詞聯想前須先出嚴格匹配」)—— 保留多長度候選但不讓長碼蓋短嚴格匹配。User 在 A/B/C 三案選 **A**(B=收斂成 walker 單段、移除「自選 chunk 長度」自由度;C=反轉覆蓋會壓掉合法 2 音節首段)。`tier` 仍在 score 之上(真 full-buffer 左錨 exact 仍高且已 vs slot0 去重,Codex Q1 OK)。reframe `within_tier_longer_coverage_beats_shorter`→`_higher_score_beats_longer_coverage` + 2 整合測試(`span_local_fetch`/`user_freq_plumb` 的 Tier-1 sub-order 翻 freq 序,headline 臺灣台語 Tier-0 #1 不變)+ 新 `single_syllable_first_segment_not_buried_by_longer_prefix` / `coverage_breaks_tie_only_when_score_and_freq_equal` bug-pin。Codex pre-impl PASS(6Q 無 BLOCK:keep tier high、demote 足夠不需第 9 維、recency flip OK、reframe 非遮 regression、engine-only confirmed、doc sweep 要更廣)。`cargo test --workspace` 全綠、clippy 0、touched-files rustfmt-clean。剩 Codex post-impl sandwich + `/codex-pr-review` + dogfood(需 `make build` 重建引擎二進位後驗) | S2 |
...
**YAGNI / non-goal(明確排除)**:MOE Nail 逐段確認 UX(2026-03 已否決)、caret/nail dual-cursor、RIME YAML 組態系統、神經 LM(離線/隱私約束)、stateful `inputLine`(維持 stateless 重算 per fetch,直到 lattice learning 真的需要 — 見 `project_moe_segmentation_audit_pending` / MOE audit §6 留 Phase 10+)。**S6 額外排除**:G1b 完整版(任意中段 custom substring 需合成非-syllabifier lattice edge)、UserVoc/LearnedVoc 手動 vs 自動學習分離(`mainstream-ime-comparison.md` §70 MOE moe_taigi_apk = future,非本片)。**S6 最佳實踐對齊**:custom 當「參與 lattice 的高信心 edge + cost rule」對齊 librime user-dict 進同一 DAG segmentation(`src/rime/dict/user_dictionary.cc`,動態權重)、McBopomofo `UserPhrasesLM` 進 Gramambular + 單音節 epsilon-boost 防壓垮(`McBopomofoLM.cpp:120`,我們已有 `WALKER_SINGLE_SYLLABLE_USER_DELTA_SCALE=0.0` 同構守門);**非** bolt-on fallback(`feedback_no_redundant_fallback` — 折進 walker provider 單一路徑,不加並行「不在 walker 就 fallback span-local」)。

**與暫停中 3 bug 的關係**:Bug 2 + §1 `taiuantaigi` **原宣稱由 S2 吞掉,實為不健全**(誠實標註,2026-05-17):S2/S3 的 `edge_score` 是全正、無 `ln(1/p)` 正規化、maximize Σ 的 objective → 結構性獎勵過度切分;`picks_high_frequency_phrase_path_over_single_chars` 測試把單字 freq 灌成 30/20(真實 6 萬+)才會過,pin 了現實不成立的前提。真實輸入 `taiuan` 實際吐 `乾伊有俺`(dogfood confirmed)。**真正修復 = S5**(min-cost 正規化 log-prob)。`no_dict_path_is_per_syllable_roman` 的逐音節期望亦不健全(真實 lattice 含 ta/i/u/an 子音節邊,finest 會是 6 段非 `tai uan tai`)→ S5 改 greedy-longest carve-out。**Bug 1(swap commit)、Bug 3(preedit 生命週期)與斷詞正交**,已各自獨立修並 merged(Bug 1 #281、Bug 3 Model B #283)。

### 連續輸入 compound-hyphen(v3.5.8 §10.2 Option A,dogfood 修復)
**Status**: ✅ DONE(branch `continuous-compound-hyphen-join`)。**Dogfood bug**:`hitetsaboo` 逐一點 `Hit`→`ê`→`tsa`→`bóo` 得 `hit ê tsa bóo`,期望 `hit ê tsa-bóo`(查某=詞庫已知 2 音節複合詞,TL/POJ 正字法詞內音節用連字號)。nice-to-have(user:不影響閱讀)。**根因**:手動逐音節 tap path 每點 nail 成獨立 `NailedSegment{syllable_count==1}`,join(`api::nailed_prefix`)分隔符只有空格或無,無複合詞感知。整句 walker path 無此 bug(複合詞=單一 edge,dict TL 內含連字號)。**修(Option A,user 採納)**:`nailed_prefix` 拆純 `nailed_prefix_with_oracle`(左→右不重疊 bigram + `prev_hyphen` chain-prevention + trailing-`-` 視為已連字號)+ 單一 `LexiconHandle::with_state`(cheap roman-ish+單音節對前置閘後);相鄰兩單音節段 `canonical_text` 串接經新 `lexicon::compound_hanji_exists`(exact `hanzi:` key,掃全 rowids,`syllable_count==2`+hanzi 再驗,非 ranker)命中→內部 `-` 取代詞界空格;未裝詞庫→graceful 純空格 join(byte-identical pre-Option-A)。**engine-only:零 proto/平台/`Intent`/`NailedSegment` 欄位**;連字號每次 render 由段自身 `canonical_text`+`syllable_count` 即時推導,守 backspace 還原不變式。限制(user 接受):純-TAILO 段不連、3+ 音節複合詞僅部分連(`tsa-bóo lâng`)。**Codex sandwich**:pre-impl **3 輪收斂無 BLOCK**(dispatch-enrich → join-time → 漢字鍵直查,逐步移除 toneless-chain 搬移);post-impl **SHIP**(1×Low test-comment overclaim → 已加直接 `lookup_exact` parity 斷言修正)。`cargo test --workspace` 綠、clippy 0、touched-files rustfmt-clean。Source-of-truth = [`continuous-commit-and-display.md`](engine/continuous-commit-and-display.md) §10.2 *Compound-hyphen*。release scope/timing 由 user 明示(`feedback_no_unilateral_release_scope`);上機 dogfood 需先 `make build` 重建引擎二進位。
