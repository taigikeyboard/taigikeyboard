...
- **詞庫增補檔案 (dev supplement) dictionary source.** A new user-toggleable source under 補充資料 (default ON), seeded with six curated reference sets — 一府五院 / 內政部菜市仔名 / 台臺 / 教典僻智識 / 數字時間日期 / 行政區 (2477 net-new words). The toggle title links to the source repository; a 「建中」整理、提供 credit is added to the 致謝 page. (#368 / #369)
