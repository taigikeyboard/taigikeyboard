...
// 行程內的 EngineHandle 單例,負責持有 Mutex<Engine> 與 generation 同步。
// generation 不一致時靜默把狀態退回 Idle,再套用本次請求。
...
// 行程內共享的組字引擎 handle,內含引擎本體與上次見到的 generation。
...
impl EngineHandle {
    ...
    // 對外的組字分派入口,先處理 generation 不一致重設,再委派給 dispatch。
    pub fn handle(
        &self,
