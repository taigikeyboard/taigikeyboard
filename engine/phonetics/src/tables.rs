...
// 中文: 共用表音資料表 (TL 聲母/韻母、POJ 替換、聲調符號)。TPS/注音表放在 tps.rs。

use once_cell::sync::Lazy;
use std::collections::{HashMap, HashSet};

// 中文: TL 聲母合法集合,用來判斷音節起頭是否合法。
...
// 中文: TL 韻母合法集合,涵蓋鼻化、入聲、雙韻、ee/er/ir 等地區變體。
...
// 中文: 聲調數字 → NFD 組合符號;1、4 聲不帶符號;此處 9 聲為 POJ 形式 (U+0306),TL 用 `tl_tone_mark` 覆蓋。
...
// 中文: 反向表 — 組合符號 → 聲調數字;9 聲同時收 POJ (U+0306) 與 TL (U+030B) 兩種寫法。
...
// 中文: TL 聲母 → POJ 聲母對應 (僅 ts/tsh 兩條規則)。
...
// 中文: TL 韻母 → POJ 韻母替換規則,順序有意義 (`nn` 必須在 `oo` 前)。
...
// 中文: 由聲調數字查 TL 用的組合符號 (9 聲走 TL 專屬的雙重銳音符)。
...
/// the table.
// 中文: 由聲調數字查 POJ 用的組合符號;9 聲為短音符 (已收在主表內)。
