...
// 中文: Phonetics 子分派;把 `PhoneticsRequest.method` 各種 oneof 變體路由到對應實作,並包成 `PhoneticsResponse`。
...
// 中文: 對解碼後的 `PhoneticsRequest` 做分派;`AppConfig` 為單次請求快照,跟著 live-read 設定一起傳入。
pub fn handle(
