...
// 中文: 自訂詞庫跨輸入模式搜尋鍵衍生 (R3)。寫入端把單一 roman 展成 tl/poj/tps × num/notone/abbrev
// 中文:   多家族鍵 (對齊系統字典三索引);查詢端依當前 input + mode 產生單一家族鍵。鍵只在
// 中文:   custom_dictionary.db 內比對,不碰系統 FST,故只要 write==query 同字即可,raw roman 路徑不動。
