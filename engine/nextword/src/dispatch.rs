...
// 中文: 把 proto 請求解碼為 Intent、分派給 Engine、再把回應打包成 proto。
// 中文: envelope 層的世代重置在 `EngineHandle::handle` 處理,不在這層。
...
// 中文: 把 proto oneof method 解碼為強型別 Intent;欄位缺失會回傳對應錯誤。
...
// 中文: 純分派入口;解碼後依方法分流至 decide / filter / boost / 狀態快照路徑。
pub fn handle(
    req: &NextWordRequest,
    engine: &mut Engine,
    config: &AppConfig,
