...
// `EngineHandle` 是 nextword 引擎的全行程單例,每行程一個 `Mutex<Engine>`。
// envelope 世代不符時靜默重置狀態並 +1 current_generation,確保在途查詢一定判定 stale。
...
// 行程單例的引擎包裝;持有引擎本體與最後一次看到的 envelope 世代值。
...
impl EngineHandle {
    ...
    // nextword 最上層分派;先做 envelope 世代檢查與重置,再呼叫純分派表。
    // 鎖順序固定先 nextword 後 last_generation,持鎖期間禁止呼叫平台 / FFI / log。
    pub fn handle(
        &self,
        req: &NextWordRequest,
        config: &AppConfig,
