...
// nextword crate 的公開門面,定義 `Engine`、`PersistedState`、`Intent`、`NextWordError`。
// 決策 / 過濾 / 評分 / 加權的實作在子模組,這裡只暴露穩定介面給 dispatch 與外部 crate。
...
/// pre-migration.
// 引擎會變更的長期狀態,與 iOS / Android 平台側結構 1:1 對齊。
...
pub(crate) struct PersistedState {
    ...
    /// a reset.
    // 單調遞增世代計數,用來丟棄過期的非同步預測結果(wrapping_add 換算非重置)。
...
// 解碼後的 intent enum,對齊 iOS / Android 平台 intent,另含 Android-only 的 Space 路徑。
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum Intent {
    // 選詞:平台主要寫入路徑,可能觸發 association 紀錄與下一輪預測。
    WordSelected {
        text: String,
        roman: String,
    ...
    // 退格:把最後一個字當作上下文,重新發查詢。
    Backspace {
        last_char: String,
        now_ms: i64,
    },
    // 上下文逾時:30 秒未動作後重置狀態並清候選詞。
    ContextTimeoutFired {
        now_ms: i64,
    },
    // 開始新的組字:藏候選詞但保留上下文;若先前在顯示則發 ClearPredictionsUI。
    ...
    // Android 限定的 Space 路徑;只更新狀態,不重排計時器、不增世代,只發複合詞 association。
    ...
    // 平台告知候選詞顯示狀態;只同步 is_showing,不發 effect 也不增世代。
    SetIsShowing {
        is_showing: bool,
    },
}
...
// 引擎錯誤;在 FFI 邊界一律映射為 `ErrorCode::FailInvariant`。
#[derive(Debug, Error)]
...
// 狀態機本體;在 FFI 邊界包進 `Mutex<Engine>` 確保跨執行緒安全。
...
impl Engine {
    ...
    // 純讀取狀態快照,不變更狀態;供 QueryState 與 envelope 重置後回報使用。
    ...
    // 套用 intent 變更狀態,回傳 effects 與最新狀態快照;Platform 未指定時回傳錯誤。
    pub(crate) fn apply(
        &mut self,
        intent: Intent,
        config: &AppConfig,
    ...
    // 評分、合併、排序、截斷原始候選列;世代不符時回傳 was_stale=true。
    pub(crate) fn filter(
        &self,
    ...
    // IME session 切換時的靜默重置;清空狀態但 current_generation 仍 +1,讓在途查詢一定過期。
