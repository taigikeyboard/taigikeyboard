...
// lexicon 操作的型別化錯誤;在 dispatch 層映射到 envelope.proto::ErrorCode。引擎絕不對使用者輸入 panic。

// lexicon 各類錯誤 enum;新增變體時記得同步更新 as_proto_error_code 對應。
#[derive(thiserror::Error, Debug)]
pub enum LexiconError {
    // 路徑含 NUL byte 不可作為檔案路徑使用。
    #[error("path `{0}` contains a NUL byte")]
    InvalidPath(String),

    // 路徑非絕對路徑;引擎不做 canonicalize,要求平台傳入完整路徑。
    #[error("path `{0}` is not absolute")]
    PathNotAbsolute(String),

    // mmap 開檔失敗 (檔案不存在 / 權限 / I/O 錯誤等)。
    ...
    // 尚未呼叫 install,EngineHandle 全域狀態為空。
    #[error("not initialized — call install before search")]
    NotInitialized,

    // 二進位格式錯誤 (magic / 版本 / offset 表 / record 長度)。
    #[error("invalid binary format: {0}")]
    InvalidBinary(String),

    // 內部不應發生的錯誤 (mutex poisoned、狀態欄位缺失等)。
...
// 將 LexiconError 映射到 envelope.proto 的 ErrorCode 整數值。
impl LexiconError {
    // 回傳對應的 proto ErrorCode (FAIL_PARSE=1 / FAIL_INTERNAL=2 / FAIL_IO=3 / FAIL_INVARIANT=4)。
