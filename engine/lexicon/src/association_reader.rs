...
// AssociationReader — TKWA bigram 二進位 mmap 讀取器。
// 以前一詞為 key 做 UTF-8 位元組二分搜尋,僅有 1 層 9-bit 來源過濾 (與 TKDB 的 3 層過濾刻意不同)。
...
// bigram 紀錄 — 「前一個詞 → 候選詞 (next_word + next_tl)」 + 出現次數 + 來源 bitmask。
#[derive(Debug, Clone)]
pub struct AssociationEntry {
    ...
    // 後續候選詞的 TL 羅馬字。
    pub next_tl: String,
    // 該 bigram 在語料中的出現次數,用於排序 (DESC)。
    pub count: u32,
    // 該紀錄的字典來源 bitmask (僅低 9 位)。
    pub bitmask: u16,
}

// TKWA mmap 讀取器,持有 mmap handle 與 key 數量等 header 資訊。
...
// bigram 來源過濾條件 — all_enabled 為 true 時略過 mask 比對。
#[derive(Debug, Clone, Copy)]
pub struct AssocFilter {
    // 所有來源皆啟用時為 true (對應 u32::MAX sentinel)。
...
impl AssociationReader {
    // 開啟並驗證 association.bin — 檢查 magic、版本與 offset 表大小。
    ...
    // 回傳 association.bin 的 build timestamp (供版本對齊驗證使用)。
    ...
    // 以前一詞做位元組二分搜尋,回傳至多 limit 筆原始順序 (依 count DESC) 的紀錄。
    ...
    // 1 層來源過濾 — all_enabled 直接通過,否則 entry_bitmask 與 enabled_mask 至少要有一個共同位元。
