...
/// Hands back the AppKit window a SwiftUI view ended up in.
///
/// For a sheet that has to raise a sheet of its own: a file panel run on the
/// SETTINGS window while a sheet is up is queued behind that sheet and never
/// appears (`NSWindow.beginSheet`), so the panel has to be given the sheet's
/// own window instead. SwiftUI does not hand that out; AppKit does, the moment
/// the view is in a hierarchy.
struct HostWindowReader: NSViewRepresentable {
    @Binding var window: NSWindow?

    func makeNSView(context _: Context) -> NSView {
        WindowReadingView(reported: $window)
    }

    func updateNSView(_: NSView, context _: Context) {}
}

/// `viewDidMoveToWindow` rather than a deferred read: it is AppKit's own answer
/// to "you are in a window now", and it fires again when the sheet closes,
/// which is what clears the binding.
private final class WindowReadingView: NSView {
    private let reported: Binding<NSWindow?>

    init(reported: Binding<NSWindow?>) {
