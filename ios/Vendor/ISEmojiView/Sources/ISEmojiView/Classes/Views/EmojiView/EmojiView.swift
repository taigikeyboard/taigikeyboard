...
public final class EmojiView: UIView {
    ...
    @IBInspectable private var countOfRecentsEmojis: Int = MaxCountOfRecentsEmojis {
        didSet {
            ...
            categoriesBottomView?.categories = emojis.map(\.category)
        }
    }
...
extension EmojiView {
    ...
    private func setupBottomView() {
        ...
        let categories: [Category] = emojis.map(\.category)
        ...
        if bottomType == .pageControl {
            ...
            pageControlBottomView = bottomView

            _bottomView = bottomView
        } else if bottomType == .categories {
            let needToShowAbcButton = keyboardSettings?.needToShowAbcButton ?? needToShowAbcButton
            let needToShowDeleteButton = keyboardSettings?.needToShowDeleteButton ?? true
    ...
    private func setupConstraints() {
        guard let emojiCollectionView, let bottomContainerView else {
            return
        }
        ...
        addConstraints(
            NSLayoutConstraint.constraints(
                withVisualFormat: "V:|-2-[emojiCollectionView]-(0)-[bottomContainerView(44)]",
                options: NSLayoutConstraint.FormatOptions(rawValue: 0),
                metrics: nil,
                views: views,
            ),
