...
class EmojiCollectionView: UIView {
    ...
    override var intrinsicContentSize: CGSize {
        CGSize(width: UIView.noIntrinsicMetric, height: frame.size.height)
    }

    // MARK: - Public

    public func popPreviewShowing() -> Bool {
        !emojiPopView.isHidden
    }
    ...
    func updateRecentsEmojis(_ emojis: [Emoji]) {
...
extension EmojiCollectionView {
    ...
    private func dismissPopView(_ usePopViewEmoji: Bool) {
        ...
        if !currentEmoji.isEmpty, usePopViewEmoji {
            delegate?.emojiViewDidSelectEmoji(emojiView: self, emoji: Emoji(emojis: emojiPopView.emojiArray), selectedEmoji: currentEmoji)
        }
