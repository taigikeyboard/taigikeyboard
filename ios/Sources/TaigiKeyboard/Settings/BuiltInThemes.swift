// 內建主題靜態表 — 三個「按鍵風格」family(經典 / 框線 / 簡潔),共用同一組 7 色(預設 + 5 light 漸層 + 1 dark 暗眠山貓)。
// 三 family 顏色完全相同,差別只在按鍵風格:經典 = 一般填色鍵;框線 = 透明鍵 + 邊框;簡潔 = 透明鍵無框。
// 透明鍵 = normalKeyFill/specialKeyFill 設成 clear(alpha 0)→ 鍵盤背景(平塗/漸層)從鍵面透出 = 「鍵=背景」。
// 5 漸層 light-only(dark = nil,深色模式仍顯 light);暗眠山貓 dark-only(light = nil,淺色模式仍顯 dark);預設保持 adaptive(bg/text 留 nil,render 端依系統 colorScheme 解析)。
...
// 單一內建主題 family — section 標題 + variant 主題(主題頁一條橫向 shelf)。三 family = 按鍵風格軸。
...
enum BuiltInThemes {
    ...
    // 攤平後的查表清單(供 resolver/SharedSettings 以 id 解析)。
    ...
    // 每個 family 的按鍵風格軸。三 family 共用顏色,只差按鍵呈現。
    ...
    // 7 個共用顏色之一。gradient == nil = adaptive 預設;接著 5 個是柔和單色相 light 漸層;最後 1 個是 dark-only 暗眠山貓(Catppuccin Mocha),放 dark slot。
    private struct BaseColor {
        ...
        // dark 主題:深漸層配 light 字 + 深中性鍵色,放 dark slot(light=nil),不隨系統明暗變。
    ...
    // 組單一 family 的 7 主題。經典 head 用 default sentinel;框線/簡潔 用 framed*/clean* id。light 主題放 light slot、dark 主題(暗眠山貓)放 dark slot。
    ...
    // 解析 (顏色, 按鍵風格) 的配色。經典 預設 = 全 nil(adaptive);框線/簡潔 預設 = 只帶透明鍵填色。
    ...
    // 漸層顏色單一 scheme variant — 2-stop 漸層 + 字色(keyText);鍵 = 中性填色(neutralFill,經典)或透明(框線/簡潔)。light/dark 主題傳入各自的 keyText/neutralFill。
