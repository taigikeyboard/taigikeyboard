// 輸入模式 (InputMode) 領域型別。POJ / TL / English / TPS 四種。
// Foundation-only 純值型別,shared-core 候選;displayName 在 SettingsModels.swift 以 extension 提供。
...
// 使用者目前選用的輸入法種類。rawValue 與 UserDefaults 持久化欄位對齊。
public enum InputMode: String, CaseIterable {
    ...
    // 英文直通模式,不走音標處理。
    case english
    // TPS 台語注音符號 (Taiwanese Phonetic Symbols)。
