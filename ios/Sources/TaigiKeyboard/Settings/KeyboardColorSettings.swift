// 鍵盤外觀顏色設定的持久化型別。Codable + UserDefaults 序列化。
// nil 欄位代表未自訂,該位置回退到 KeyboardKit 預設的動態 (light/dark) 顏色。
...
// 可序列化的單一 RGBA 顏色。light/dark 共用同一組值 (刻意設計);未自訂時整個欄位為 nil,讓 KeyboardKit 動態色生效。
struct CodableColor: Codable, Equatable {
    ...
    // 從 SwiftUI Color 透過 UIColor 萃取 RGBA 分量,寫入持久化欄位。
    init(_ color: Color) {
    ...
    // 從 0xRRGGBB 直接算 RGBA(alpha=1),高 8 bits 忽略。內建主題表專用,免字串解析、無失敗路徑。
    init(hex: UInt32) {
...
// 鍵盤背景的垂直漸層(top→bottom),≥2 stops 才渲染;<2 退回平面 backgroundColor。內建漸層主題用,平面主題 nil。
...
// 鍵盤六個可自訂顏色面;任何欄位為 nil 即代表「沿用 KeyboardKit 預設」。
struct KeyboardColorSettings: Codable, Equatable {
    ...
    // 是否有可渲染的漸層(≥2 stops)。render 分支 / liquid-glass gate / 候選列透明 共用此單一判斷。
...
extension CodableColor {
    ...
    // 把顏色往白提亮(各分量 + (255-分量)×factor 取整),維持不透明。用來推導第一候選 highlight(主題淺色)。
    ...
    // 把顏色往黑加深(各 0-255 分量 × factor 取整),維持不透明。0-1→0-255→0-1 來回是刻意的:
    // 強制逐 byte 整數截斷,與 Android deepenedArgb 完全一致(跨平台 byte 對齊),勿簡化。
