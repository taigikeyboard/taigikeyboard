// 中文: 設定與使用者資料的 reset 中介層。把 SharedSettings 與 KeyboardSettings.store 兩個來源接起來,
// 中文: 並把破壞性 reset (清空使用者頻率/關聯 DB) 與單純還原預設值刻意分開兩個 entry point。
...
// 中文: Reset 協調器。SharedSettings 不能直接 import KeyboardKit (engine-only 模組約束),
// 中文: 因此跨 store 的 reset 邏輯統一在這裡組合。
enum SettingsResetCoordinator {
    ...
    // 中文: 還原 KeyboardKit 自有的三個 store key (自動大寫 / 音效回饋 / 震動回饋)。
    // 中文: 必須與 SettingsTab「全部重設」UX 同步。
    ...
    // 中文: 重設所有設定 (SharedSettings + KeyboardKit) 為預設值,但不動使用者資料 DB。
    ...
    // 中文: 刪除使用者頻率 DB 與 next-word 關聯 DB。兩個刪除動作獨立進行,任一失敗都只記 log,
    // 中文: 不向上拋,避免單一失敗讓另一邊也跟著失敗。
