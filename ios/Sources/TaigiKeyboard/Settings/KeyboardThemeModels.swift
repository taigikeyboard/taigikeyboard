// 鍵盤主題模型 — 主題身分 id、內建主題、使用者自訂主題、解析後的渲染主題。
// BuiltInTheme(內建主題)於 PR-2b 加入,提供 light/dark 兩套 6 角色顏色。
...
// 主題身分。"default" = 既有自由配色 buffer(SharedSettings.colorSettings);
// 未自訂用戶解析為全 nil → KeyboardKit adaptive + Liquid Glass。其餘 id 為
// UserTheme 的 UUID 字串(PR-3)或 built-in 主題 id(PR-2b)。
enum ThemeId {
    ...
    // id 是否為自訂主題(自訂 = UUID;default / built-in 不是)。用來判斷主題是否自帶外觀(含明確陰影)。
...
// 內建主題(唯讀,隨 app 打包)。每個主題帶 light/dark 兩套具名 6 角色配色,
// 渲染時依系統 colorScheme 解析;某一 variant 為 nil 時 fallback 另一套。
struct BuiltInTheme: Equatable {
    ...
    // 顯示名稱的 i18n key,在 picker call site 用 DisplayLanguageStore 解析,跟隨使用者選的顯示語言。
    ...
    /// swatch.
    // 卡片預覽截圖的 asset 名(尺寸對齊齒盤佈局頁)。nil 則退回即時色塊 swatch。
    ...
    // 選用的鍵邊框寬度 override。內建主題以配色為主,但可帶這一個外觀 scalar(框線 family 用),resolver 疊在原廠尺寸上;nil = 原廠 0(無框)。
    ...
    // 依 colorScheme 取對應 variant;缺一套時退到另一套;兩套皆缺才退 .default。
...
// 同一個型別同時是「UserTheme 儲存的內容」「resolver 回傳的結果」「render 讀的值」,避免各欄到處平鋪。
struct ThemeAppearance: Codable, Equatable {
    ...
    /// decoding.
    // 原廠外觀。內建主題(只定義配色)以此為底;decode 缺欄位也退回這裡。
...
// 向前相容 decode 放在 extension,讓 struct 仍自動合成 memberwise init
// (ThemeAppearance.default / legacyAppearance / 測試等皆以 memberwise 建構)。
extension ThemeAppearance {
    ...
    // 缺欄位退回 default,未來新增外觀欄位不會讓舊主題檔解碼失敗。
    init(from decoder: Decoder) throws {
        ...
        // 字型改為全域設定後不再屬於主題。舊 user_themes.json 帶 "fontType" key 仍可解碼(Codable 忽略未知 key)。
    }
}
...
// 不需向後相容 decode — user-theme 寫入路徑(CRUD)從未 ship,也無 seeding 寫過檔,
// 故無 legacy 格式存在;未來欄位成長由 ThemeAppearance 的 decodeIfPresent 向前相容處理。
