// 中文: POJ 預處理的兩個 Bool 開關打包成一個值型別,讓 ComposingState / ToneConverter 不必直接讀設定。
// 中文: Foundation-only,shared-core 候選。
...
// 中文: ToneToggles 是引擎呼叫 phonetics 時必填的 POJ 預處理參數。
// 中文: 由 EngineSettingsProvider live-read 取出,再以值型別方式傳入引擎。
public struct ToneToggles: Equatable {
    // 中文: 雙擊 OO 開關 (POJ ↔ TL 互轉時的 oo 處理)。
    public let isDoubleTapOOEnabled: Bool
    // 中文: 雙擊 NN 開關 (鼻化音 nn 的處理)。
