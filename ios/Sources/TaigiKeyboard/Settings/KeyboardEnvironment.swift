// keyboard extension UI 端設定 protocol。是 EngineSettingsProvider 的非引擎對偶 —
// 涵蓋外觀、inputMode 寫入、full-access 狀態、以及給 didChangeNotification 用的 App Group UserDefaults。
...
// UI 層設定 protocol。讀取為 live-read,渲染熱路徑請改用 snapshot(for:) 取一致快照。
// 引擎層禁止依賴此 protocol,改走 EngineSettings/EngineSettingsProvider (Foundation-only)。
protocol KeyboardEnvironment: AnyObject {
    // 目前輸入模式 (UI 端可寫入,host app 設定頁也走這個 setter)。
    var inputMode: InputMode { get set }
    // Full Access 狀態旗標 (持久化於 App Group)。
    var isFullAccessEnabled: Bool { get set }
    // 鍵盤六個顏色面 (背景 / 文字 / 鍵帽 / 候選列 等)。外觀編輯器讀寫此 buffer。
    var colorSettings: KeyboardColorSettings { get }
    // 渲染端消費的解析外觀(選定主題 + colorScheme → 6 顏色 + 陰影 + 5 尺寸)。字型為全域設定,不在此包內。
    // "default" 走全域外觀;built-in 依 colorScheme 取 light/dark。colorScheme 來源 = keyboardContext.colorScheme。
    ...
    // 鍵盤字體選用 (system / openHuninn / iansui / genYoMin / genYoGothic)。全域設定,非每主題。
    // 供 callout / KeyboardFonts.globalFont 等無 per-render snapshot 的呼叫點直接讀取。
    var fontType: FontType { get }
    // 鍵盤排版 (phahTaigi / qwerty / tps / moe1 / moe2)。
    ...
    // App Group 共享的 UserDefaults。controller 對它註冊 didChangeNotification,
    // 收到 host app 寫設定後即時刷新 keyboard extension 狀態。
    var settingsUserDefaults: UserDefaults { get }

    // 取得當下渲染週期的設定一致性快照,避免渲染 ~50 個鍵時重複讀 UserDefaults。
    // colorSettings 欄位依 colorScheme 解析(built-in 主題的 light/dark 在此決定)。
    func snapshot(for colorScheme: ColorScheme) -> SettingsSnapshot
}

extension SharedSettings: KeyboardEnvironment {
    // 將 SharedSettings 持有的 App Group store 暴露給 protocol 觀察者。
