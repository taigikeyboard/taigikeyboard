// 自訂詞庫條目的新增 / 編輯表單畫面。
// 同檔提供 create / edit 兩個 init,onSave callback 由父畫面寫回 ViewModel。

import SwiftUI

/// Add/Edit form for a custom dictionary entry
// 自訂詞庫單筆編輯畫面。羅馬字 + 漢字兩欄;canSave 需兩欄都非空白。
struct CustomDictionaryEditView: View {
    ...
    /// Edit mode
    // 編輯模式 — 帶入既有條目作為 prefill,儲存時保留原 id 與 createdAt。
    ...
    // 儲存按鈕動作 — trim 兩欄、新增/編輯模式各自組裝 entry,呼叫 onSave 後 dismiss。
