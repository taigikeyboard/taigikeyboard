// DataManagementView 的 ViewModel — 整體備份檔(.taigi)的匯出 / 匯入。
// SwiftUI fileExporter / fileImporter 綁定仍由 View 持有,本 VM 只提供 payload 與 service 呼叫。
...
final class DataManagementViewModel: ObservableObject {
    ...
    // 從 URL 讀檔並交給 BackupService 匯入,回傳各分類匯入結果。
