// CustomDictionaryView 的 ViewModel — 自訂詞庫 CRUD + CSV 匯入匯出 + 啟用開關。
// 表單欄位狀態(輸入框、alert flag)仍由 View 自行持有。
...
// 自訂詞庫畫面的 ViewModel,封裝 CustomDictionaryService 操作給 View 綁定。
@MainActor
final class CustomDictionaryViewModel: ObservableObject {
    ...
    // 自訂詞庫啟用開關;切換時同步寫回 SharedSettings。
