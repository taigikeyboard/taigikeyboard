// CustomDictionary / Frequency / Association 三個資料管理畫面共用的匯入匯出 handler。
// 集中管理 fileImporter / fileExporter / progress / 各式 alert 的狀態,View 透過
// importExportModifiers() 一次掛上 SwiftUI modifiers。
...
// 匯入匯出流程的 ObservableObject;含進度旗標、CSV document 暫存、各式 alert 訊息。
@MainActor
final class ImportExportHandler: ObservableObject {
    // 匯入進行中旗標,View 顯示 progress UI 用。
    @Published var isImporting = false
    // 觸發 SwiftUI fileImporter sheet。
    @Published var showFileImporter = false
    // 觸發 SwiftUI fileExporter sheet。
    @Published var showFileExporter = false
    // 待匯出的 CSV 文件實體;在 performExport 完成後填入。
    @Published var csvDocument: CSVDocument?

    // 匯入結果 alert 開關 + 顯示文字(由 formatResult closure 格式化)。
    ...
    // 預設錯誤格式化 — 直接顯示 localizedDescription。單一來源,供 handleFileImport / showError 共用。
    ...
    // 把 csvGenerator() 產生的字串包成 CSVDocument 並觸發 fileExporter sheet。
    ...
    // 統一處理 fileImporter 結果 — 設定 isImporting、跑 importAction、組訊息、彈 alert。
    ...
    // 產生帶日期後綴的匯出檔名,格式 "<prefix>_yyyy-MM-dd.csv"。
    ...
    // 設定錯誤訊息並彈出 error alert。formatError 在顯示邊界把錯誤轉成(可能本地化的)字串。
...
// 私有 ViewModifier — 一次掛上 fileImporter / fileExporter / 三類 alert。
...
extension View {
    // 便利 API — 在 View 上一次掛載匯入匯出相關的所有 SwiftUI modifiers。
