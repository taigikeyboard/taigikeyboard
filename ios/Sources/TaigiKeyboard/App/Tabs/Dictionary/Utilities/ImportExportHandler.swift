...
final class ImportExportHandler: ObservableObject {
    ...
    // 中文: 匯入結果 alert 開關 + 顯示文字(由 formatResult closure 格式化)。
    ...
    // 中文: 預設錯誤格式化 — 直接顯示 localizedDescription。單一來源,供 handleFileImport / showError 共用。
    ...
    // 中文: 設定錯誤訊息並彈出 error alert。formatError 在顯示邊界把錯誤轉成(可能本地化的)字串。
