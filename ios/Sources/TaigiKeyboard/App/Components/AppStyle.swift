// 主 App UI 樣式常數的集中定義(字級、顏色、間距、圓角等)。
// 所有 Tab / Settings 共用,改這裡即可全 App 套用。詳見 .claude/rules/ui-style-guide.md。
...
enum AppStyle {
    ...
    // 取指定點數的 Open Huninn 字型。所有 App UI 字型都該走這個或下方語意屬性。
