// 鍵面文字 provider — 渲染鏈第二棒,接手 ButtonImageProvider 沒處理的鍵。
// 負責大小寫轉換、顯示覆寫(˙→·、nn→ⁿ)、調符 hint、TPS callout hint、
// MOE 標點 hint、鍵盤切換鍵文字、組字模式的 return 鍵文字。
...
final class ButtonTextProvider {
    ...
    // MOE1 / MOE2 標點鍵的 hint 對照;isTextHint 與 buttonHintText 共用同一張表避免漂移。
    ...
    // 是否為 TPS callout hint(hint 字較大、主鍵字較小)。
    ...
    // 區分文字 hint(較小字)與獨立調符 hint(較大字),供 standardHintContent 決定字級。
    ...
    // 數字鍵上方調符的垂直偏移;tone 8 (ˈ) 字符基線較低,用較小 offset 保持與鍵面字距離一致。
    ...
    // 鍵面上方的 hint 文字;TPS callout、數字鍵調符、MOE 標點 hint 三類。
    // 英文模式或非 MOE 佈局回 nil。
    ...
    // 回傳鍵面文字;nil 讓 KeyboardKit 用預設內容。
    ...
    // 0-9 數字鍵上的調符 hint;沒有可見調符的鍵(0/1/4)回空白佔位以維持垂直對齊。
    // 第 9 鍵 POJ 用 breve(˘),TL 改用 double prime(ʺ)以提升辨識度。
    ...
    // 字元鍵的最終顯示文字 — 套用顯示覆寫(˙→·、TPS 全形逗號、POJ 的 nn→ⁿ/ᴺ)
    // 與大小寫轉換(capsLocked / uppercased / lowercased)。
    ...
    // 組字模式中 Enter 鍵顯示的確認字 — TPS / 譯字模式為「選」,POJ 為「soán」,TL/英文為「suán」。
