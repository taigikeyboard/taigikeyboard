...
// 中文: 兩條字型解析路徑 — 鍵面字 (ButtonFontProvider) 與工具列/候選 UI (globalFont / globalUIFont),
// 中文: 都依 SharedSettings.shared.fontType.customFontName 解析使用者選擇的字型。
...
// 中文: 字型工具命名空間 — 自訂字型的 PostScript 名稱常數 + 全域字型解析。
enum KeyboardFonts {
