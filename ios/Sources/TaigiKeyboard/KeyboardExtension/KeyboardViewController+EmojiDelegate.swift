// KeyboardViewController 對 EmojiService 的 delegate 實作 — emoji 選取、切回字母鍵盤、退格、關閉鍵盤。
...
extension KeyboardViewController: EmojiServiceDelegate {
    ...
    // 點選 emoji 時走 ComposingManager.commitPreeditThenInsertExternal,
    // 讓正在組字的 preedit 與 emoji 一起 atomic 送出,避免靜默清掉 preedit。
    ...
    // emoji 鍵盤的退格走標準退格路徑,讓組字 / 閒置兩種分支與一般鍵盤一致。
