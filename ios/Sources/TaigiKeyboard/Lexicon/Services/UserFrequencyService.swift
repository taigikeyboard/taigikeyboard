// 使用者詞頻 service — 對 UserFrequencyRepository 做薄包裝,提供候選詞流程使用。
...
final class UserFrequencyService: @unchecked Sendable {
    ...
    // 確保底層頻率 DB 已打開且 schema 套用 — 第一次查詢前先呼叫,避免 cold-connect 延遲。
    ...
    // 紀錄候選 commit;R5 身分 = (word, tl) pair(#7),tl 為 canonical TL 讀音。
