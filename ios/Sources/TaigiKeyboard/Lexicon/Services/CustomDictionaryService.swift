// 中文: 自訂詞庫的 service 層 — CRUD、CSV 匯出、檔案匯入。底下委派給 CustomDictionaryRepository。

import Foundation

/// Service for managing user custom dictionary
/// Handles CRUD, CSV export, and file import
// 中文: 自訂詞庫服務 — 把 repository 包成適合 UI / settings 流程使用的介面。
final class CustomDictionaryService: @unchecked Sendable {
    ...
    // 中文: 詞庫為空時補上預設範例,讓使用者第一次看到自訂詞庫頁有東西可看。
    ...
    // 中文: 從本地 CSV 檔匯入詞條 — 含檔案大小、entry 數量、編碼、格式四道前置驗證。
    ...
    // 中文: 解析 CSV 字串成詞條 — 第一欄 = roman,第二欄 = hanzi。空行 / 不足兩欄會被跳過。
    ...
    // 中文: CSV 輸出時若欄位內含逗號 / 引號 / 換行,要包雙引號並對引號做 escape。
