// 中文: 自訂詞庫的搜尋鍵衍生入口 — legacy notone / abbrev / roman_num(write-only 回滾用)
// 中文: 以及 v3.6.1 R3 跨模式搜尋鍵 bundle(寫)/ 單一家族查詢鍵(讀)。
...
// 中文: 包住 Rust 引擎的自訂詞搜尋鍵衍生;legacy 欄位保留 write-only,跨模式鍵走側表。
