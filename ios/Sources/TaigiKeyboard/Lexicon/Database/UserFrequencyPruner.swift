// user_frequency 表的 pruning 政策 — 軟上限、判斷頻率、批次刪除大小,
// 篩選順序與 Android 一致(count ASC, last_used ASC)。
...
enum UserFrequencyPruner {
    ...
    // 多砍一些 row,讓表在下一次 prune 前能維持在上限以下,
    // 不會在 cap 附近震盪。
    ...
    // 超量才執行 prune。count + delete 在同一個 execute block 內,
    // 保證讀寫一致,不需要第二個 transaction。
    @discardableResult
    ...
    // 總 row 數,prepare 失敗回 -1 — 對齊 repository.totalCount() 對連線壞掉的契約。
