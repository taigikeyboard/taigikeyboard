// 把 TL 顯示形(帶調符)轉成數字調並組成外部辭典查詢 URL。
...
// URL 慣例屬於平台層(percent-encoding / query 參數),所以這裡刻意不搬到 Rust;
// 只有調符前處理與 strip-tone 走 RustEngineBridge。
enum ExternalLookupURLBuilder {
    /// Chhoe Taigi dictionary lookup URL for the given TL display form.
    // Chhoe Taigi 辭典查詢 URL。
    ...
    // 把 TL 顯示形轉成數字調形,例:tāi-tsì → tai7-tsi3。
    ...
    // 單音節 diacritic → 數字調轉換。1 / 4 聲在 URL 中省略,
    // 與 Chhoe Taigi、MOE Sutian 的 query 慣例一致。
