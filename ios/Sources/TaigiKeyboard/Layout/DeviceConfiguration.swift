// 中文: 鍵盤佈局用的裝置/螢幕分類工具。
// 中文: ScreenSizeClass 依螢幕寬度判別 phoneCompact / phoneRegular / phoneLarge / pad,
// 中文: DeviceConfiguration 額外快取 Touch ID 偵測(iOS 17+ 上 = iPhone SE)。

import LocalAuthentication
import UIKit

/// Screen size classification based on device screen width
// 中文: 依螢幕寬度做的尺寸分類 — 影響 layout 選擇與 callout 行為。
enum ScreenSizeClass {
    // 中文: iPhone SE / iPhone mini(寬 < 375)。
    case phoneCompact // iPhone SE, iPhone mini (width < 375)
    // 中文: 標準 iPhone(375 ≤ 寬 < 414)。
    case phoneRegular // Standard iPhone (375 ≤ width < 414)
    // 中文: iPhone Plus / Max(寬 ≥ 414)。
    case phoneLarge // iPhone Plus/Max (width ≥ 414)
    // 中文: iPad。
    case pad // iPad

    // 中文: 由 UIScreen 量測當前裝置取得的尺寸分類。
...
// 中文: 給 layout 取得「是否 iPad / 是否小尺寸 iPhone」的工具。
struct DeviceConfiguration {
    ...
    // 中文: 快取 — 生物辨識型別屬硬體層級,執行期不變。iOS 17+ 上只剩 iPhone SE 用 Touch ID。
