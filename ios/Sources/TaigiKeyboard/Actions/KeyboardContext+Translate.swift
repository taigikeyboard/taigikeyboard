// 為 KeyboardKit KeyboardContext 加上漢字 / 羅馬字顯示切換的擴充,後端寫進 SharedSettings。
...
public extension KeyboardContext {
    ...
    // 候選詞顯示模式。鍵盤內 overlay 改值走這裡,與 文/A 切換共用同一條重繪路徑。
    ...
    // 只有 漢羅並排 才切換;羅馬字 / 漢羅濫 為 no-op — 鍵仍顯示,但不改 stored 值。
