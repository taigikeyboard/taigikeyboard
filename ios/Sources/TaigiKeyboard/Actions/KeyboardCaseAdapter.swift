// KeyboardKit Keyboard.KeyboardCase 與 Rust bridge CaseTransformLetterCase 之間的轉接器。
// 隔離 KeyboardKit 型別,讓引擎側不會直接接觸 Keyboard.KeyboardCase。
...
// KeyboardCase → CaseTransformLetterCase 的對映擴充。
extension Keyboard.KeyboardCase {
    // 把 KK 的鍵盤大小寫狀態映射成 bridge 端的 LetterCase 三態。
