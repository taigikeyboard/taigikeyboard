// 展開候選 overlay 的 row 排版工具。
// 依預先量測好的寬度把 suggestions 拆成多行,寬度不夠就換行。
...
// 展開候選的 wrapping row 排版命名空間 — 不歸 shared-core,所有量測由外部注入。
enum ExpandedCandidateRowLayout {
    // 一個候選詞在排版後的位置資訊 — 原始 index + 量測寬度。
    struct RowItem {
    ...
    // 把 suggestions 依寬度排成多行 — 累計寬度超過 availableWidth 就換行。
