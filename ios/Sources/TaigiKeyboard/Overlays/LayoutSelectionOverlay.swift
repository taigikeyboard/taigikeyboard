// 中文: 從工具列叫出的「鍵盤佈局選擇」overlay。
// 中文: 把可用佈局以卡片陳列,讓使用者不必離開鍵盤就能切換 layout。
...
// 中文: 鍵盤佈局選擇面板 — 與 ExpandedCandidateOverlay 採用相同的 overlay 樣式。
struct LayoutSelectionOverlay: View {
    ...
    // 中文: 寫入 SharedSettings 並排程短暫延遲後自動收合 overlay。
