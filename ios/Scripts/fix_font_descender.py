...
"""
字型 Descender 修復腳本
修復 jf-openhuninn-2.1.ttf 的 descender 截斷問題

使用方法：
python3 fix_font_descender.py
...
def verify_fix(fixed_font_path):
    """驗證修復效果"""
    print(f"\n🔍 驗證修復效果...")
    ...
    print(f"  與原始 -153 相比: 增加 {descender_space - 153} 單位 ({((descender_space - 153) / 153 * 100):.1f}%)")
    print(f"  hhea 與 OS/2 差異: {abs(hhea.descent - os2.sTypoDescender)} 單位")

    font.close()

def update_project_font(fixed_font_path):
    """更新專案中的字型檔案"""
    project_font_path = "jf-openhuninn-2.1.ttf"

    if os.path.exists(project_font_path):
        # 備份專案中的原字型
        backup_path = backup_original_font(project_font_path)

        # 複製修復後的字型到專案
        shutil.copy2(fixed_font_path, project_font_path)
        print(f"✅ 專案字型已更新: {project_font_path}")
        print(f"📁 原字型備份: {backup_path}")

        return True
    else:
        print(f"❌ 找不到專案字型檔案: {project_font_path}")
        return False
...
def main():
    ...
    print("🎯 jf-openhuninn 字型 Descender 修復工具")
    print("=" * 60)
    ...
    if response == 'y':
        print("\n📦 步驟 5: 更新專案字型")
        success = update_project_font(fixed_font_path)
